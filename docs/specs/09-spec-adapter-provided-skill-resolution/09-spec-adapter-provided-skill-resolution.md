...
- The transitional `HarnessAdapter.loadSkill()` method is explicitly deprecated in `packages/engine/src/adapter.ts`; implementation should remove or replace it only to the extent required for issue #12 without expanding unrelated lifecycle redesign.
