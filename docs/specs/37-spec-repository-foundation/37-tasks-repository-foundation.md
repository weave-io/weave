...
## 7. Fresh baseline (G8) — PR: #241 (7.1), #BPR (7.2–7.4)
...
- [x] 7.2 Run the development subset with repeats on current `main`: `weave eval run --models dev --repeat 3`. It must complete without infrastructure errors. Record its time and cost, and the per-case flip rate (how often a case's verdict differs between repeats; was 19.1). Check OpenRouter credits first. _#BPR, on `d4d1bf17`: 276 attempts, 0 errored, 85% passed, $0.18; about 1 h 40 min as one process (it was split into 10 parallel units by suite, 23 min wall clock, because of a 10-minute limit per tool call). 16 of 92 case × model pairs (17%) flipped across the three repeats. See [the baseline](../../artifacts/eval-baseline-2026-09-24.md)._
- [x] 7.3 Run the full default matrix once on current `main`, with the trajectory job (20.2). Record its time and cost. _#BPR: the text track on all 8 default models (368 attempts, 64 parallel units, 0 errored, 89% passed, 23 min wall clock, $6.92). The trajectory track ran on the dev subset only (the 5 cases DeepSeek V4 Flash may run, all passed, 5 min, $0.04); the full-matrix trajectory run (4 cases on Sonnet 4.5, Opus 5 and GPT-5.5) was not part of this baseline's plan and has not been run._
- [x] 7.4 Record the pre-WS1 baseline in `docs/artifacts/eval-baseline-<date>.md`: per-suite pass rates, their noise bands, the per-case flip rates, the judge id and version, and the time and cost of each run. Later runs are compared against it with `eval compare` (18.2). Publishing it to the website is not required. _#BPR: [`eval-baseline-2026-09-24.md`](../../artifacts/eval-baseline-2026-09-24.md), judge `typesafe/jev-1.13-20260917`. It lists three suspected defects without fixing them: `review_blocker_traced` fails on every model, `pattern-plan-verify-by-per-criterion` contradicts the composed Pattern prompt, and a passing trajectory case is labelled `fail`. The bundles stay local; the artifact says where._
