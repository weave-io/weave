 ...
 * path. The ref is validated against the store's own bounded, contained,
 * root-relative grammar immediately before serialization, and anything else
 * is omitted.
 */
...
function showHandlerFor(sessionRef: string) {
...
describe("children.show never serializes an unproven session ref", () => {
  ...
  test("a valid ref is still reported under --diagnostic", async () => {
    const output = (
    ...
    expect((JSON.parse(output) as Record<string, unknown>).sessionRef).toBe(
      REF,
    );
  });
