...
- **Publish** (optional). Only when `WEAVE_EVAL_PUBLISH_MODE=publish` and a results-repo token are set, which in practice means the CI workflow. Only the allowlisted public files (`bundle-index.json`, `public-report.json` and `public-report.md`) and the derived dashboard indexes are pushed to `weave-io/weave-agent-evals`, and [tryweave.io/evals](https://tryweave.io/evals) reads them from there. The text run and the trajectory run each keep their own pointer (`latest.json` and `latest-trajectory.json`), so the trajectory job, which publishes second, never replaces the text run the page shows; `weave eval reindex` rebuilds the indexes from the published runs if they ever go wrong ([Track-aware indexes](eval-sanitization-and-publish-pipeline.md#track-aware-indexes)). The score files and the provenance manifest are never published: they stay in the local bundle, which in CI is uploaded as a workflow artifact kept for 30 days. `raw/` is never published either, and CI never writes it. See [Eval Sanitization and Publish Pipeline](eval-sanitization-and-publish-pipeline.md).
