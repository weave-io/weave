...
oversized native output (P007), explicit cleanup with tombstones (P008),
settings validation including rejected legacy quota keys (P009), bounded native
session paging without full reads (P011), and CLI `children.show` paging through
`readSessionEntryPage` only with opaque cursors (P012).
...
native Pi child sessions under the Weave XDG root. Paging and CLI show evidence
bind to HEAD fixes that preserve chronological multi-chunk pages and the
adapter-command / public-package boundary.
