...
**Related:** [ADR 0001: Prompt Composition Templates](../adr/0001-prompt-composition-templates.md) · [Builtin Prompt Guidelines](../contributing/builtin-prompts.md) · [Adapter Boundary](../architecture/adapter-boundary.md) · [Config Loading](configuration.md) · [Tool Policy Evaluation](tool-policy.md) · [TypeScript Conventions](../contributing/typescript.md) · [Context Glossary](../../CONTEXT.md) · [CLI — `weave prompt self-modify`](cli.md#weave-prompt-self-modify)
...
This boundary follows [Adapter Boundary](../architecture/adapter-boundary.md): the engine owns
...
The `weave prompt self-modify` guide enforces this: it lists `docs/reference/prompts.md` as a required pre-read for prompt-related changes. See [CLI — `weave prompt self-modify`](cli.md#weave-prompt-self-modify).
...
and size contract is defined in [Builtin Prompt Guidelines](../contributing/builtin-prompts.md).
...
5. **Resolve tool policy**
   ...
   - This produces a complete `EffectiveToolPolicy` with all five abstract
     capabilities resolved. See [Tool Policy Evaluation](tool-policy.md).
...
[TypeScript Conventions](../contributing/typescript.md).
