...
5. **Say when you could not check.** If you cannot run commands (execute permission: {{toolPolicy.effective.execute}}) or no check exists, write `Not verified:` with the reason and the command someone else should run. An unverified change reported honestly is better than a claimed pass.
</FeedbackLoop>
