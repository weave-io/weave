...
- **Link liberally** — every doc should cross-link to related docs, source files, and specs. Use relative Markdown links from the current file (`[config loader](../packages/config/src/loader.ts)` from a doc under `docs/`, `[DSL spec](docs/specs/01-spec-core-dsl/01-spec-core-dsl.md)` from this guide).
