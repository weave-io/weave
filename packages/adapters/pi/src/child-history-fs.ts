...
export type PiChildHistoryFsError =
...
export interface PiChildHistoryDirectory {
  readonly path: string;
  identity(): ResultAsync<HistoryIdentity, PiChildHistoryFsError>;
  readFile(
    name: string,
  ): ResultAsync<Uint8Array | undefined, PiChildHistoryFsError>;
  writeFileAtomic(
    name: string,
    bytes: Uint8Array,
    mode: number,
  ): ResultAsync<void, PiChildHistoryFsError>;
  appendFile(
    name: string,
    bytes: Uint8Array,
    mode: number,
  ): ResultAsync<void, PiChildHistoryFsError>;
  deleteFile(name: string): ResultAsync<void, PiChildHistoryFsError>;
  quarantineFile(
    name: string,
    suffix: string,
  ): ResultAsync<void, PiChildHistoryFsError>;
...
export interface PiChildHistoryFsPort {
  openDirectory(
    path: string,
    create: boolean,
  ): ResultAsync<PiChildHistoryDirectory, PiChildHistoryFsError>;
}

type ResolvePiChildHistoryRootInput = {
...
export function safeParentSessionComponent(
  parentSessionId: string,
): Result<string, PiChildHistoryFsError> {
  if (
    parentSessionId.length <= MAX_COMPONENT &&
    SAFE_COMPONENT.test(parentSessionId) &&
    parentSessionId !== "." &&
    parentSessionId !== ".."
  ) {
    return ok(parentSessionId);
  }
  return ok(
    new Bun.CryptoHasher("sha256").update(parentSessionId).digest("hex"),
  );
}

export function safeChildHistoryComponent(
  childId: string,
): Result<string, PiChildHistoryFsError> {
  if (childId.length === 0) return err({ type: "unsafe-path" });
  if (
    childId.length <= MAX_COMPONENT &&
...
export function resolvePiChildHistoryRoot(
  ...
  const dataHome = configured ?? join(actualHome, ".local", "share");
  return safeParentSessionComponent(value.parentSessionId).map((component) =>
    join(dataHome, "weave", "adapters", "pi", "child-history", component),
  );
}
...
function sameIdentity(left: HistoryIdentity, right: HistoryIdentity): boolean {
...
function mapThrownError(value: unknown): PiChildHistoryFsError {
  return isFsError(value) ? value : { type: "io" };
}

function fsErrAsync<T = never>(
  error: PiChildHistoryFsError,
): ResultAsync<T, PiChildHistoryFsError> {
  return errAsync<T, PiChildHistoryFsError>(error);
}

function fsVoidAsync(): ResultAsync<void, PiChildHistoryFsError> {
  return okAsync<void, PiChildHistoryFsError>(undefined);
}
...
function loadNative(): Result<NativeLibc, PiChildHistoryFsError> {
...
function absoluteSegments(
  path: string,
): Result<readonly string[], PiChildHistoryFsError> {
...
function statFd(
  ...
  kind: "directory" | "file",
  expectedMode: number,
): ResultAsync<HistoryIdentity, PiChildHistoryFsError> {
  return ResultAsync.fromThrowable(
    ...
    if (
      ...
      return errAsync<HistoryIdentity, PiChildHistoryFsError>({
        type: "wrong-kind",
        kind,
      });
    }
    ...
    return okAsync<HistoryIdentity, PiChildHistoryFsError>({
      dev: stat.dev,
      ino: stat.ino,
      mode,
    });
...
function openDirectoryChain(
  ...
  create: boolean,
  mode: number,
): ResultAsync<number, PiChildHistoryFsError> {
...
class BunHistoryDirectory implements PiChildHistoryDirectory {
  constructor(
    readonly path: string,
    private readonly libc: NativeLibc,
    private readonly fd: number,
    private readonly bound: HistoryIdentity,
  ) {}

  private readonly fileBounds = new Map<string, HistoryIdentity>();

  private checkFileIdentity(
    name: string,
    identity: HistoryIdentity | undefined,
  ): Result<HistoryIdentity | undefined, PiChildHistoryFsError> {
          ...
          statFd(fresh, "directory", 0o700)
            ...
            .andThen((identity) =>
              sameIdentity(identity, held)
                ? okAsync<HistoryIdentity, PiChildHistoryFsError>(held)
                : errAsync<HistoryIdentity, PiChildHistoryFsError>({
                    type: "identity-changed",
                  }),
            ),
      );
  ...
  private name(name: string): Result<string, PiChildHistoryFsError> {
    return safeName(name) ? ok(name) : err({ type: "unsafe-path" });
  }

  private targetIdentity(
    name: string,
  ): ResultAsync<HistoryIdentity | undefined, PiChildHistoryFsError> {
    ...
    if (fd < 0) {
      return this.libc.errno() === ERRNO_ENOENT
        ? okAsync<HistoryIdentity | undefined, PiChildHistoryFsError>(undefined)
  ...
  private atomicReplace(
    name: string,
    bytes: Uint8Array,
    expected: HistoryIdentity | undefined,
  ): ResultAsync<void, PiChildHistoryFsError> {
    return ResultAsync.fromThrowable(async () => {
      const temporary = `.${name}.tmp-${crypto.randomUUID()}`;
      const fd = this.libc.openat(
        this.fd,
        cstr(temporary),
      ...
      let installed = false;
      try {
        if (this.libc.fchmod(fd, 0o600) !== 0) throw { type: "io" };
        let offset = 0;
        ...
        if (
          ...
          (expected !== undefined &&
            (current === undefined || !sameIdentity(expected, current)))
        ) {
          throw { type: "identity-changed" };
        }
        if (
          this.libc.renameat(this.fd, cstr(temporary), this.fd, cstr(name)) !==
          0
        ) {
          throw { type: "io" };
        }
        installed = true;
        if (this.libc.fsync(this.fd) !== 0) throw { type: "io" };
        const installedIdentity = await unwrapResult(this.targetIdentity(name));
        if (installedIdentity === undefined) throw { type: "identity-changed" };
        this.fileBounds.set(name, installedIdentity);
      } finally {
        if (!closed) this.libc.close(fd);
        if (!installed) this.libc.unlinkat(this.fd, cstr(temporary), 0);
      }
    ...
    return this.identity().andThen(() => {
      ...
      if (file < 0) {
        if (this.libc.errno() === ERRNO_ENOENT) {
          return okAsync<Uint8Array | undefined, PiChildHistoryFsError>(
            undefined,
          );
        }
          ...
          this.identity()
            ...
            .andThen((current) =>
              current !== undefined && sameIdentity(fileIdentity, current)
                ? okAsync<Uint8Array, PiChildHistoryFsError>(bytes)
                : errAsync<Uint8Array, PiChildHistoryFsError>({
                    type: "identity-changed",
                  }),
            ),
        );
  ...
  writeFileAtomic(
    ...
    return this.identity()
      ...
      .andThen((expected) =>
        this.atomicReplace(checked.value, bytes, expected),
      );
  }
  ...
  quarantineFile(
    ...
    const checked = this.name(name);
    if (checked.isErr()) return errAsync(checked.error);
    const safeSuffix = safeName(suffix) ? suffix : "quarantine";
    return this.identity()
      .andThen(() => this.checkedTargetIdentity(checked.value))
      .andThen((target) => {
        if (target === undefined) return fsErrAsync<void>({ type: "missing" });
        return this.identity()
          .andThen(() => this.targetIdentity(checked.value))
          .andThen((current) => {
            if (current === undefined || !sameIdentity(target, current)) {
            ...
            const quarantine = `.${checked.value}.quarantine-${safeSuffix}-${crypto.randomUUID()}`;
            return this.libc.renameat(
              this.fd,
              cstr(checked.value),
              this.fd,
              cstr(quarantine),
...
class BunPiChildHistoryFs implements PiChildHistoryFsPort {
  openDirectory(
    path: string,
    create: boolean,
  ): ResultAsync<PiChildHistoryDirectory, PiChildHistoryFsError> {
    ...
    return openDirectoryChain(loaded.value, path, create, 0o700)
      .andThen((fd) =>
        statFd(fd, "directory", 0o700)
          .map(
            (identity) =>
              new BunHistoryDirectory(path, loaded.value, fd, identity),
          )
          .mapErr((error) => {
...
function nextMemoryIdentity(mode = 0o700): HistoryIdentity {
  memoryInode += 1;
  return { dev: 1, ino: memoryInode, mode };
}

export class MemoryPiChildHistoryFs implements PiChildHistoryFsPort {
  ...
  openDirectory(
    ...
    const fs = this;
    const fileBounds = new Map<string, HistoryIdentity>();
    let closed = false;
    ...
    const handle: PiChildHistoryDirectory = {
      path,
      identity(): ResultAsync<HistoryIdentity, PiChildHistoryFsError> {
        ...
        return this.identity().andThen(() => {
          ...
          return this.identity().andThen(() => {
            ...
            return current !== undefined &&
              ...
              ? okAsync<Uint8Array, PiChildHistoryFsError>(
      ...
      writeFileAtomic(name, bytes, mode) {
        ...
        return this.identity().andThen(() => {
          ...
          current.files.set(name, next);
          fileBounds.set(name, next.identity);
      ...
      quarantineFile(name, suffix) {
        ...
        return this.identity().andThen(() => {
          ...
          const existing = current.files.get(name);
          if (existing === undefined) return fsErrAsync<void>({ type: "missing" });
          const checked = validateMemoryFile(existing);
          if (checked.isErr()) return errAsync(checked.error);
          const known = checkFileIdentity(name, existing.identity);
          if (known.isErr()) return errAsync(known.error);
          ...
          current.files.delete(name);
          fileBounds.delete(name);
          current.files.set(`.${name}.quarantine-${safeSuffix}`, {
            ...existing,
            identity: nextMemoryIdentity(0o600),
...
function validateMemoryFile(
  file: MemoryFileData,
): Result<MemoryFileData, PiChildHistoryFsError> {
