...
[permission contract](permissions.md),
[ADR 0009](../adr/0009-input-aware-tool-permission-authorization.md),
[Runtime Store contract](runtime.md), and
[execution lifecycle contract](execution-lifecycle.md).
This page orients; it does not redefine those rules.

**Related:** [Adapter Boundary](../architecture/adapter-boundary.md) · [Tool Policy Evaluation](tool-policy.md) · [Engine package README](../../packages/engine/README.md)
...
This subsystem is available to adapters, but the Pi adapter does not use it.
Pi tool calls stay under Pi and each concrete tool owner's native control. The
Pi adapter creates no permission registry, session, challenge, grant, permit,
coverage proof, approval UI, or interception hook.
...
`authorizeCall`/`authorizeCaptured` evaluation. It is adapter-owned whether a given registered call reaches that evaluation at
all. Adapters that do not implement the permission subsystem, including Pi,
leave calls under the harness or tool owner's control.
...
Before claiming Weave-enforced `tool-policy-mapping` readiness, adapters call
