...
[Spec 33](specs/33-spec-pi-adapter/33-spec-pi-adapter.md) defines full readiness for `@weaveio/weave-adapter-pi` on interactive Earendil Works Pi TUI sessions. The runtime, diagnostics, packed-package proof, and nightly release integration now exist. The acceptance manifest, exact-host compatibility matrix, and digest-bound stable TUI smoke-checklist machinery are implemented and checked in (see [`docs/specs/33-spec-pi-adapter/acceptance-manifest.json`](specs/33-spec-pi-adapter/acceptance-manifest.json) and [`33-smoke-checklist.md`](specs/33-spec-pi-adapter/33-smoke-checklist.md)); the live TUI smoke pass itself has not been executed, so every requirement row's `result` stays `pending`.
...
| Stable interactive TUI readiness | ⏳ Pending | Acceptance manifest, exact-host compatibility matrix, and digest-bound smoke checklist are implemented and every mandatory `PI-*` row's named tests/packed proof verify clean; the live TUI smoke pass has not been executed, so `result` is `pending` on all 20 rows |

Pi reports static declarations as ceilings and lowers them through one activation probe per capability. Any required effective degraded or unsupported capability enters health-only mode. The adapter must not claim ready until every requirement row's `result` is flipped to `pass` after the digest-bound live TUI smoke checklist is actually run against the exact released artifact (see [`docs/specs/33-spec-pi-adapter/33-smoke-checklist.md`](specs/33-spec-pi-adapter/33-smoke-checklist.md)).
...
| [Spec 33](specs/33-spec-pi-adapter/33-spec-pi-adapter.md)                                                 | Full-readiness Pi Adapter           | Defines Pi's TUI-only package, lifecycle, projection, proof, and release contract; acceptance manifest/matrix/checklist machinery is implemented, live smoke evidence is pending |
