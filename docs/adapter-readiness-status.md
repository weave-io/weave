...
[Spec 33](specs/33-spec-pi-adapter/33-spec-pi-adapter.md) defines full readiness for `@weaveio/weave-adapter-pi` on interactive Earendil Works Pi TUI sessions. The runtime, diagnostics, packed-package proof, nightly release integration, and exact-host live smoke evidence are complete. All 20 acceptance-manifest rows and all 23 digest-bound TUI checks passed against Pi `0.81.1`. See the [`acceptance-manifest.json`](specs/33-spec-pi-adapter/acceptance-manifest.json), [`33-smoke-checklist.md`](specs/33-spec-pi-adapter/33-smoke-checklist.md), and [`33-task-12-proofs.md`](specs/33-spec-pi-adapter/33-proofs/33-task-12-proofs.md).
...
| Stable interactive TUI readiness | ✅ Ready | All 20 mandatory `PI-*` rows pass, and S001–S023 passed against the manifest's exact package digest, source revision, attempt, checklist version, and Pi `0.81.1` host |

Pi reports static declarations as ceilings and lowers them through one activation probe per capability. Any required effective degraded or unsupported capability enters health-only mode. The completed checklist proves the ready path and the health-only failure path against the same bound artifact.
...
| [Spec 33](specs/33-spec-pi-adapter/33-spec-pi-adapter.md)                                                 | Full-readiness Pi Adapter           | Defines Pi's TUI-only package, lifecycle, projection, proof, and release contract; all manifest rows and exact-host live smoke checks pass |
