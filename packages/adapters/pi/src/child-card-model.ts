...
export function projectDelegationCardFacts(
  ...
  return {
    ...
    activity:
      state.activity?.kind === "fallback"
        ? state.activity
        : { kind: "boot", text: "", live: false },
...
function toneOf(state: PiDelegationCardState): PiCardTone {
  ...
  if (
    state.activity?.kind === "error" ||
    state.phase === "tool error" ||
    state.phase === CARD_PROVIDER_ERROR_PHASE
  ) {
    return "bad";
  }
  if (
    state.activity?.kind === "queue" ||
    state.activity?.kind === "fallback" ||
    state.phase === "steered"
  ) {
    return "warn";
