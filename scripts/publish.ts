...
async function main() {
  ...
  for (const pkg of PACKAGES) {
    ...
    try {
      ...
      if (dryRun) {
        const result = await $`bun publish --access public --dry-run`
          .cwd(pkgPath)
          .nothrow();
        if (
          result.exitCode !== 0 &&
          !result.stderr.toString().includes("authentication")
        ) {
      ...
      const result = await $`bun publish --access public --tag ${tag}`
        .cwd(pkgPath)
        .quiet();
      ...
      if (
        ...
        console.log(
          `  ⊘ ${pkgJson.name}@${pkgJson.version} already published, skipping`,
        );
      } else {
