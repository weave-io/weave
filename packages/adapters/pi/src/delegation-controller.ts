...
export interface PiDelegationControllerDeps {
  ...
  /** Generation-scoped parent-card reasoning sink. */
  readonly onParentCardReasoning?: PiLiveReasoningObserver;
  /** Generation-scoped inspector reasoning sink. */
  readonly onInspectorReasoning?: PiLiveReasoningObserver;
  /** Generation-scoped bounded transient reasoning registry. */
...
export interface PiThreadRunRequest {
  ...
  /** Receives each authenticated model transition for this run. */
  readonly onModelTransition?: (transition: PiModelTransitionBody) => void;
  /** UI-only parent-card reasoning sink for this resume run. */
  readonly onParentCardReasoning?: PiLiveReasoningObserver;
  /** UI-only inspector reasoning sink for this resume run. */
  readonly onInspectorReasoning?: PiLiveReasoningObserver;
  /** Optional registry override for this resume run. */
  readonly liveReasoningRegistry?: PiLiveReasoningRegistry;
  /**
...
export interface PiDelegationRequest {
   ...
   */
  readonly onSessionEvent?: (event: PiChildSessionEvent) => void;
  /** Receives each authenticated model transition for this child. */
  readonly onModelTransition?: (transition: PiModelTransitionBody) => void;
  /** UI-only parent-card reasoning sink; never receives session events. */
  readonly onParentCardReasoning?: PiLiveReasoningObserver;
  /** UI-only inspector reasoning sink; independent from the parent sink. */
  readonly onInspectorReasoning?: PiLiveReasoningObserver;
  /** Optional registry override for this child. */
  readonly liveReasoningRegistry?: PiLiveReasoningRegistry;
  /**
...
export class PiDelegationController {
        ...
        return mintedSession.andThen((runSession) => {
          ...
          return this.appendThreadDivider(
            ...
            const runRequest: PiDelegationRequest = {
              ...
              ...(request.onModelTransition === undefined
                ? {}
                : { onModelTransition: request.onModelTransition }),
              ...(request.onParentCardReasoning === undefined
                ? {}
                : { onParentCardReasoning: request.onParentCardReasoning }),
              ...(request.onInspectorReasoning === undefined
                ? {}
                : { onInspectorReasoning: request.onInspectorReasoning }),
              ...(request.liveReasoningRegistry === undefined
                ? {}
                : { liveReasoningRegistry: request.liveReasoningRegistry }),
            };
  ...
  /** Delivers one authenticated model transition to the caller sink. */
  private invokeModelTransitionCallback(
    childId: string,
    callback: ((transition: PiModelTransitionBody) => void) | undefined,
    transition: PiModelTransitionBody,
  ): void {
    ...
    Result.fromThrowable(
      ...
      (code) => {
        this.deps.logger.warn(
          { childId, code },
          "delegation onModelTransition callback failed",
        );
      },
    );
  }
  ...
  private handleChildModelTransition(
    childId: string,
    transition: PiModelTransitionBody,
    onModelTransition?: (transition: PiModelTransitionBody) => void,
  ): void {
    const state = this.findLiveThreadState(childId);
    ...
    this.invokeModelTransitionCallback(childId, onModelTransition, transition);
    this.notifyTreeChanged();
  }
    ...
    const child = new PiRpcChild(
      ...
      depth,
      {
        ...
        onModelTransition: (transitionChildId, transition) =>
          this.handleChildModelTransition(
            transitionChildId,
            transition,
            request.onModelTransition,
          ),
        ...
        onParentCardReasoning:
          request.onParentCardReasoning ?? this.deps.onParentCardReasoning,
        onInspectorReasoning:
          request.onInspectorReasoning ?? this.deps.onInspectorReasoning,
        liveReasoningRegistry:
          request.liveReasoningRegistry ?? this.deps.liveReasoningRegistry,
      ...
      .andThen(({ session }) => {
        ...
        const child = new PiRpcChild(
          ...
          1,
          {
            ...
            onParentCardReasoning: this.deps.onParentCardReasoning,
            onInspectorReasoning: this.deps.onInspectorReasoning,
            liveReasoningRegistry: this.deps.liveReasoningRegistry,
          },
        );
        const spawn = {
          childId,
