...
if (args[0] === "--help") {
  log.info(
    "usage: release-control <invocation.json> <manifest.json> <artifact-directory>",
  );
  process.exit(0);
}
if (args.length !== 3) {
  log.error("expected invocation, manifest, and artifact directory");
  process.exit(2);
}
const [invocationPath, manifestPath, artifactDirectory] = args;
...
if (invocationText.isErr() || manifestText.isErr()) {
