...
| Date | 24 Sep 2026, 18:17–19:43 UTC |
...
# Phase 4 — the trajectory half of the full matrix: the same five cases on the other
# default models they allow (15 units: 5 cases × Sonnet 4.5, Opus 5, GPT-5.5)
TMPDIR=~/.cache/weave-trajectory-tmp \
  bun packages/cli/src/main.ts eval run --agent <suite> --case <case> --model <model> --track trajectory --repeat 1
```
...
| 4. Default matrix, trajectory, `--repeat 1` (models not in phase 2) | 15 | 0 | 7 min 21 s (15 units, 5 at a time) | 31 min | $3.41 |
| **Total** | | **0** | **≈ 1 h 2 min** | | **$10.55** |

The totals include a $0.004 timing probe, three trajectory sessions that were re-run because their first launch lacked `--agent` (see [Trajectory outcomes](#phase-2-trajectory-outcomes)), and two single-case diagnosis runs. The whole session stayed well inside its $25 budget. Phase 4 by model: Sonnet 4.5 $0.33, Opus 5 $1.60, GPT-5.5 $1.48. Phase 3 by model pair, as measured: GPT 6 Astra + Claude Opus 5 $3.28, Claude Opus 4.5 + GPT-5.5 $2.11, Claude Sonnet 4.5 + GPT-5.6 Sol $0.98, Qwen3.8 Max + DeepSeek V4 Flash $0.55.
...
## Phase 4: full-matrix trajectory cases

The default-matrix trajectory run (task 7.3, the trajectory job of 20.2) covers the same five cases on every default model in their `allowed_models`: DeepSeek V4 Flash (phase 2), Claude Sonnet 4.5, Claude Opus 5 and GPT-5.5. Each case ran once on each model, one unit per case, on the same commit. **17 of 20 passed**, and every session completed without a harness error.

| Case | DeepSeek V4 Flash | Sonnet 4.5 | Opus 5 | GPT-5.5 |
| --- | --- | --- | --- | --- |
| `loom-delegates-backend-fix-to-category-trajectory` | PASS | FAIL: no sub-agent spawned, 5 tool calls, execution 0.50 | PASS | PASS |
| `loom-runs-the-failing-check-itself-trajectory` | PASS¹ | PASS¹ | PASS¹ | PASS (delegated to `shuttle`) |
| `shuttle-verify-tests-after-edit-trajectory` | PASS | FAIL: execution 0.67 | FAIL: execution 0.67 | PASS |
| `tapestry-dispatches-independent-tasks-in-parallel-trajectory` | PASS | PASS | PASS | PASS |
| `tapestry-runs-plan-verification-trajectory` | PASS | PASS | PASS | PASS |

¹ Loom did the work itself without delegating. The verdict is pass, but the bucket is labelled `fail` (see defect 3).

The two Shuttle failures both score routing and delegation 1.0 and execution 0.67, so one of the case's execution checks (a test run after the last edit, or the hidden verifier) did not hold. A single-case diagnosis with `--raw-artifacts` names which one.
...
- **Trajectory:** 17 of 20 sessions passed across phases 2 and 4 (5 cases × 4 models), with no harness errors. The failures are Sonnet 4.5 not delegating the backend fix to the category shuttle, and Sonnet 4.5 and Opus 5 missing one of Shuttle's execution checks. Each case ran once per model, so none of this is a noise-adjusted result.
...
> Defects 1–3 were resolved on 24 Sep 2026 (task 16.7); see [Corrections](#corrections-24-sep-2026) for the classification, the fix and the corrected pass rates. The text below is kept as it was recorded.
...
1. **`weft-review-traced-true-positive`: a detector stricter than the case.** Evidence: the task (the case description and the runner's user message) asks for "one BLOCKER: line per blocking issue with a backticked file reference", and the case's stated intent is a blocker "citing both the discarding call site and the fallible function it calls". Every answer inspected (2 diagnosis answers, 6 dev re-run answers, 8 full-matrix answers) rejected the change for the right bug, cited the call site's path, and named the fallible function `saveSettings`, in all but one answer without its path. `isTracedFinding` accepted only a second *path*, so 15 of those 16 answers failed the signal; one Luna answer happened to cite two paths. Even the #170 prompt that introduced the case let a reviewer "cite the path and the function or symbol" when naming the far end. **Fix:** a Weft blocker is also traced when it cites a file path and names a function that, in the case's own code, the cited file calls and the case declares (`extractCodeMaterial` and `isTracedThroughDeclaredSymbol` in `packages/cli/src/evals/judgment-cases.ts`). Naming only the function that contains the cited line (`runSettings`), a type the case never declares (`ResultAsync`), or a function the cited file never calls (the test only spies on `saveSettings`) still does not count, and the per-BLOCKER file requirement (`review_rejection_disciplined`, the backstop for Jev's blind spot) is untouched. The rubric notes now say the fallible function may be cited by path or by name. Warp's traced check is unchanged.
2. **`pattern-plan-verify-by-per-criterion`: a case that contradicted the product's prompt.** Evidence: the case required a `verify by:` clause on every acceptance criterion and a `- [ ]` checklist under `## Verification`. Neither the task text nor the runner's user message asks for either. The composed Pattern prompt says to use `- [ ]` only for executable top-level tasks and describes `## Verification` as the commands to run and what passing looks like. #182 removed both behaviours from the prompt on purpose, because per-criterion verify-by commands fed the verification loops that kept sessions running for hours, and kept the #170 cases; its results artifact already expected this case to fall back. So the case measured behaviour the product decided not to want. It is not a WS1 finding: changing the prompt to pass it would reintroduce what #182 removed. **Fix:** the case keeps its id and task text and now requires what the Pattern prompt asks for: explicit scope, file-backed tasks, per-task acceptance, a `## Verification` section that names at least one declared command in any form (new signal `plan_verification_uses_declared_commands` in `pattern-planning-runner.ts`), and no invented command. The rubric notes tell the judge not to require verify-by clauses or checkboxes. The diagnosis showed the new signal still separates plans: one GPT 6 Luna plan left out `## Verification` altogether, which the prompt's template requires, and missed it.
3. **`loom-runs-the-failing-check-itself-trajectory`: labels computed from a different number than the verdict.** Evidence: the verdict comes from the gating dimensions (execution 1.0 passes the case), but the bucket and the explanation came from `weightedTotal` alone, which is 0.33 when routing and delegation score 0 because Loom ran the check itself. The same split affected text cases: re-scoring every scored row in the four baseline phases with the new code changes the bucket of 28 of 667 rows, all passed cases (24 bucketed `partial`, these 4 bucketed `fail`); no failed case had been bucketed `pass`. A failed case at 0.5–0.9 was also explained as "partially passed". **Fix:** a case's bucket now follows its verdict (`computeCaseScoreBucket` in `report-schema.ts`, used for the published case entry and for the explanation): `pass` if it passed, `partial` or `fail` by total if it failed. The explanation text follows `passed` ("passed", "failed with a partial score", "failed"), and the stdout report prints the pass mark only beside a FAIL. The verdict and the case are unchanged, so routing and delegation still score 0 when Loom does not delegate; the rubric says delegation is expected but not required, and only execution gates.

Each fix has scenarios in `tests/evals/`, and each was watched go red against a mutation of the fix: `suite-runners.scenario.test.ts` ("Weft traces a blocker to the function the change calls", the Verification-section rows, and a plan written to the Pattern prompt's own template passing the real case), `scoring.scenario.test.ts` ("a reader compares a case's verdict with its band and explanation", a grid over both sides of every bar) and `bundle-writing.scenario.test.ts` (the published band follows the verdict). A unit test in `loom-routing-runner.trajectory.test.ts` reproduces defect 3 through the trajectory runner.

### Re-run of the two text cases

| Case | Dev subset, `--repeat 3` (before → after) | Full matrix, `--repeat 1` (before → after) |
| --- | --- | --- |
| `weft-review-traced-true-positive` | DeepSeek 0/3 → 3/3, Luna 2/3 → 3/3 | 0/8 → 8/8 |
| `pattern-plan-verify-by-per-criterion` | DeepSeek 0/3 → 3/3, Luna 0/3 → 3/3 | 0/8 → 7/8 |

The one full-matrix miss is Claude Opus 4.5 on the Pattern case: it wrote pseudo tool calls to read the files instead of a plan (execution 0.20), the same non-plan reply the #170 results recorded for other models. That is a single sample of model behaviour, not a scoring defect.

### Corrected pass rates

Phase 1, dev subset, `--repeat 3` (the two suites that change, and the total):

| Suite | DeepSeek V4 Flash | GPT 6 Luna | Both (pooled) |
| --- | --- | --- | --- |
| weft-review | 12/12 100% [76–100] | 12/12 100% [76–100] | 24/24 100% [86–100] |
| pattern-planning | 10/12 83% [55–95] | 6/12 50% [25–75] | 16/24 67% [47–82] |
| **All suites** | 128/138 93% [87–96] | 117/138 85% [78–90] | 245/276 89% [84–92] |

Phase 3, full default matrix, `--repeat 1`:

| Suite | Opus 4.5 | Opus 5 | Sonnet 4.5 | DeepSeek V4 Flash | GPT-5.5 | GPT-5.6 Sol | GPT 6 Astra | Qwen3.8 Max | All |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| weft-review | 4/4 | 4/4 | 2/4 | 4/4 | 4/4 | 4/4 | 3/4 | 4/4 | 29/32 (91%) |
| pattern-planning | 2/4 | 4/4 | 4/4 | 4/4 | 2/4 | 4/4 | 3/4 | 4/4 | 27/32 (84%) |
| **All suites** | 39/46 (85%) | 44/46 (96%) | 41/46 (89%) | 45/46 (98%) | 44/46 (96%) | 43/46 (93%) | 42/46 (91%) | 46/46 (100%) | 344/368 (93%) |

Trajectory verdicts are unchanged (17 of 20 passed in phases 2 and 4). Re-scored with the new code, `loom-runs-the-failing-check-itself-trajectory` is now `pass`, "required harness-trajectory case passed", on all four models (DeepSeek V4 Flash, Sonnet 4.5 and Opus 5 at a total of 0.33, GPT-5.5 at 1.00), where the baseline published `fail` for the first three.

What changes in [What the baseline says](#what-the-baseline-says): with its unpassable case measured correctly, `weft-review` is saturated on the dev subset (100%) and at 91% on the full matrix. `pattern-planning` is still the weakest suite on the dev subset (67%, with GPT 6 Luna at 50%), now on genuine misses in its other cases; `shuttle-execution` (67% dev, 79% full) joins it. Suspected defect 4 (`loom-route-weft-boundary-downstream-review` on GPT 6 Luna) was out of scope here and is still open.

## How to compare against this baseline

The bundles are local and are not in the repository. They were copied, without `raw/`, to `~/source/weave-worktrees/eval-baseline-2026-09-24/` on the maintainer's machine, laid out as `<phase>/<unit>/d4d1bf1-2026-09-24-001/` (phases `p1` to `p4`), with each unit's `run.log`, the unit timings (`timings.log`) and the credit readings (`credits.log`). `eval compare` reads only these bundles, so it needs them and makes no model call.
