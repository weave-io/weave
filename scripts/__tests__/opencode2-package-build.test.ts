...
import packageManifest from "../../packages/adapters/opencode2/package.json";
import {
...
describe("OpenCode 2 package build", () => {
  const build = PUBLIC_PACKAGE_BUILDS["@weaveio/weave-adapter-opencode2"];

  it("ships separate server, RPC, and Solid TUI entries with declarations", () => {
    expect(build.entries.map((entry) => entry.output)).toEqual([
      "packages/adapters/opencode2/dist/index.js",
      "packages/adapters/opencode2/dist/server.js",
      "packages/adapters/opencode2/dist/rpc.js",
      "packages/adapters/opencode2/dist/tui.js",
    ]);
    ...
    expect(build.declarations.map((entry) => entry.output)).toContain(
      "packages/adapters/opencode2/dist/rpc.d.ts",
    );
    expect(build.declarations.map((entry) => entry.output)).toContain(
      "packages/adapters/opencode2/dist/tui.d.ts",
    );
