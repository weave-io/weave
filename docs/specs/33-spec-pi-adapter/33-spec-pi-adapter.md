...
Alt+A cycles only materialized descriptors whose mode is `primary` or `all`, in materialization order. It never selects a `subagent`. Before the first turn it changes the pending primary; after activation it applies the next descriptor atomically. The adapter rejects manual cycling during a live direct workflow step, in health-only or child mode, or when fewer than two eligible primaries exist. Activation failure preserves the prior primary and badge. Because Pi tool schemas are static after registration, the root `weave_delegate` schema MUST expose the ordered union of normalized targets declared by cycleable primaries. Each resolver and execution MUST still read the live primary identity and that descriptor's target set, reject targets unavailable to the active primary, and use the same live identity in both the root delegation request and signed child bootstrap.
...
The adapter MUST pass the child's descriptor prompt, resolved model, active tool set, permission bridge, correlation, and bounded context. For a directly dispatched workflow step, the signed bootstrap MUST carry the activated agent's own `composedPrompt` as system context, while the lifecycle call's ephemeral `stepPromptText` (§14) MUST travel separately as the bounded RPC task. The adapter MUST NOT duplicate or concatenate the descriptor prompt into the task. A legacy/fallback dispatch with no configured step prompt uses one fixed bounded instruction to start the turn. This preserves the ordinary delegation-task bound even when a canonical primary prompt, such as Tapestry's, already reaches that bound. It MUST stream events, account usage, propagate abort, and clean all descendants. Any model identity placed in a bootstrap or bootstrap-ack control body MUST contain only `provider`/`id`/optional `name`; the strict schema rejects host-only fields. Before calling the child's `setModel()`, the adapter MUST resolve that compact identity to one exact full entry in the child's authenticated model catalog. Missing or duplicate entries fail closed.

Every bootstrap delegation target MUST preserve the normalized engine shape: `name`, optional `description`, `triggers`, and `isCategory`. Each trigger MUST preserve `domain`, `trigger`, and optional bounded `routing_hint`. The private control schema MUST accept those fields and reject unknown fields. In particular, it MUST NOT project away or reject `routing_hint`; built-in delegation targets use it, and a strict schema that omits it prevents a delegation-capable direct-step agent such as Tapestry from acknowledging bootstrap.

The root and nested `weave_delegate` registrations MUST expose `agent` as an enum of exact normalized subagent names taken from the invoking descriptor's `delegationTargets`. Tool descriptions and prompt guidance MUST require those enum values and MUST NOT accept display labels, descriptions, or aliases. A direct-step child with eligible targets receives `weave_delegate` but remains outside the ordinary delegation budget itself. Once the direct transport authenticates a running child's `delegate-request`, it MUST relay the request into the generation's shared `PiDelegationController` with the direct child ID, step-agent identity, depth, and cwd supplied by trusted transport state rather than child-controlled fields. The controller MUST resolve the requested normalized target name against that step agent's own `delegationTargets`, place the nested helper under the direct child ID, enforce the ordinary depth/concurrency/process/queue limits, and return exactly one correlated `delegate-response`. The helper MUST receive an ordinary bootstrap without `weave_complete_step`. Cancelling the direct child MUST cancel this ordinary descendant subtree as well.

A bootstrap body's `correlationId` field authenticates the child to itself: the child-side extension instance fails closed (disposes the runtime and never sends `bootstrap-ack`) unless it equals that child's own env-derived, authenticated `childId`. This value MUST always be the transport's own freshly generated `childId` - the same value passed to `PiRpcChild`'s constructor and used to derive the child's `WEAVE_CHILD_ID` environment variable - never a caller-supplied, engine-level correlation identifier such as a workflow step's own dispatch-effect correlation ID (`PiWorkflowController`'s own audit/effect correlation, §14). Conflating the two silently breaks every affected bootstrap: the child always fails closed on `correlationId` mismatch, the parent's own bootstrap-ack wait times out, and the caller observes only a generic "the delegated child did not reply in time" failure with no child process left to inspect. Ordinary delegation's bootstrap builder and direct workflow-step dispatch's bootstrap builder MUST both set `correlationId` to the generated `childId`; any engine-level correlation identifier the caller wants preserved MUST travel in its own, separate field, never overloading this one.
...
The root `weave_delegate` tool's own registered `execute()` MUST observe its five-argument `signal` (§12.2) and be the thing that turns host-level abort/interrupt into `PiDelegationController.cancelSubtree(childId)` for the exact generated child - the controller never learns of an abort on its own, and no other layer calls `cancelSubtree()` on the ordinary-helper path. This wiring MUST cover both races, never fabricate a result on its own, and never leak the abort listener:

*   **Already aborted before dispatch.** If `signal.aborted` is already `true` before the tool ever builds the delegation request, it MUST fail closed with a structured `ChildAbortFailed`-coded result and MUST NOT call `controller.delegate()` at all - there is no child yet to cancel.
*   **Mid-flight abort.** Once `controller.delegate(request)` is in flight, an abort MUST call `cancelSubtree(childId)` for that exact child and no other node. A successful `cancelSubtree()` MUST NOT itself resolve the tool call - only the same child's own eventual settlement (delivered through the same `delegate()` promise via `PiRpcChild.completeCancellation()`, §11.5) may, so the tool's returned settlement is always the child's real terminal state, never a synthesized one.
*   **Cancellation failure.** If `cancelSubtree()` itself fails (e.g. the child process is already gone), the tool MUST resolve promptly with that failure's code instead of continuing to wait on a `delegate()` settlement that may never arrive; it MUST NOT log the failure and keep waiting regardless (a hang behind a broken cancellation is itself a bug, not a degraded-but-safe outcome).
*   **No leak.** The abort listener MUST be removed on every terminal path - normal settlement, a mid-flight cancellation, or a before-dispatch failure - so a tool call that already settled is never re-entered by a later abort on the same signal.

A delegated child's own *nested* `weave_delegate` relay tool (the one built for a child that itself declares delegation targets) has no symmetric wiring, and this is intentional rather than an oversight: the wire protocol has no primitive for a child to request cancellation of a `delegate-request` it has already sent to its own parent (§11, `PiChildRuntime.admitVerifiedEnvelope` only ever admits `cancel` envelopes travelling parent-to-child, never child-to-parent). Cancelling that nested request is only possible from the root down, by cancelling the ancestor subtree that contains it; the relay tool's own `execute()` therefore has nothing to attach an abort listener to and MUST remain unwired unless a future host or wire-protocol revision adds an equivalent child-to-parent cancellation signal.
...
The injected `ChildProcessPort` MUST expose two distinct termination operations, never only one: a cooperative/default `kill()` (the underlying runtime's default signal, e.g. `SIGTERM`) and a mandatory `forceKill()` (a `SIGKILL`-equivalent signal whose disposition can never be caught, blocked, or ignored, and which the kernel delivers even to a stopped process instead of leaving it pending). Every terminal resource-cleanup path in the private child's own lifecycle - normal settlement, every failure, disposal, and above all the bounded cancellation grace period elapsing - MUST use `forceKill()`, never the cooperative `kill()`, so a stopped (e.g. `SIGSTOP`'d) or otherwise non-cooperative child can never survive its own parent's cleanup. A plain default-signal `kill()` at this single terminal cleanup step is insufficient: it can leave a non-cooperative child alive and unreaped (observable as a stopped process in `ps`) even after the delegating parent has already reported a structured `{outcome: "cancelled"}` settlement.
...
A direct-step child's settlement `observeSession` (`idle`) MUST be recorded the moment its one structured completion candidate arrives - strictly before the corresponding `completeStep` call. A terminal step's `completeStep` releases the active `ExecutionLease` as part of that same call, and the Runtime Store deletes the released lease row; observing settlement any later than that risks recording a snapshot against an already-deleted lease and degrading with `LifecycleProjectionFailed`/`persistence:query` even though the workflow itself completed (#21 Task 12). This ordering applies identically whether the candidate settles immediately or is withheld pending `/weave:advance` for a `user_confirm` step: the settlement observation is never deferred to `confirmStep`.

Each of `dispatchStep`, `completeStep`, and `reconcileExecution` also returns an ephemeral, top-level `stepPromptText?: string` alongside its effects — the rendered `step.prompt` text for the step about to be dispatched. This is never part of a `LifecycleEffect` (`RunAgentEffect.promptMetadata` continues to carry only `{ byteLength }`), never persisted, and never logged. The controller MUST thread `stepPromptText` from the lifecycle call that produced it straight into the corresponding `runDispatchAgentEffect` call as the direct dispatch's separate bounded `taskPrompt`. The activated agent's own `composedPrompt` remains a separate bootstrap/system field. The controller MUST NOT write either text back into an effect, the Runtime Store, a recovery pointer, or a log line.
...
A fresh generation's in-memory active-workflow tracker starts empty on every reload, restart, switch, or fork (S019); this MUST NOT itself resume or spawn anything even though the durable pointer file survives. Only inside the explicit `/weave:resume` handler, and only when the tracker has no in-memory instance of its own, the dispatcher MAY reconstruct `{ workflowInstanceId, leaseId, controllerGeneration }` correlation from the newest eligible pointer to hand the resume path something to inspect (S020). Reconstruction MUST fail closed—produce no correlation at all—unless the pointer's `status` is `"recoverable"` and both `workflowId` and `leaseId` are present; a terminal pointer, or one missing either field, never seeds anything.

A reconstructed pointer alone is not sufficient to resume: the pre-reload lease it names is very likely still unexpired, held by the prior controller generation's owner ID, and an ordinary `resumeExecution` call from a fresh owner conflicts on it every time (the exact S020 regression). Explicit takeover closes this gap without weakening lease semantics. `ResumeExecutionInput` accepts an optional `recoveryTakeover: { expectedLeaseId, expectedOwnerId }` correlation (`packages/engine/src/execution-lifecycle/resume.ts`). Inside one Runtime Store transaction, the engine: rejects a terminal `WorkflowInstance` outright; finds the currently active lease for the instance; if none is active (already expired or released), proceeds with an ordinary fresh acquire; if one is active, it must match `workflowInstanceId` plus both `expectedLeaseId` and `expectedOwnerId` *exactly*, else the whole operation fails closed with a typed `lease_conflict` error and zero mutation. Only on an exact match does the engine release the old lease (using its own true owner, never the caller's) and acquire a fresh lease for the new owner. There is no broad foreign-lease stealing and no automatic resume — the transaction is opt-in per call, and omitting `recoveryTakeover` preserves the exact prior behavior (only an already-expired lease is replaced). Because a takeover releases a lease the caller does not itself own, the engine additionally requires `authorizationSource` to be the explicit, literal `"user"` value whenever `recoveryTakeover` is present — a missing or non-`"user"` source fails closed with a typed validation error before any store access, even though ordinary resume without `recoveryTakeover` still defaults a missing source to `"user"`.

On the Pi side, this correlation is wired end to end but only reaches the engine after an explicit, fresh user confirmation. `activeInstanceFromRecoveryPointer` (`packages/adapters/pi/src/recovery-pointer.ts`) now also returns the pointer's `controllerGeneration`, and `PiActiveWorkflowTracker.getActiveInstance()`/`setActiveInstance()` carry it as an optional field — present only when an instance was reconstructed from a durable pointer, never for an ordinarily tracked in-session instance. `handleWeaveResume` (`packages/adapters/pi/src/workflow-commands.ts`) forwards `{ expectedLeaseId: leaseId, expectedControllerGeneration: controllerGeneration }` as `PiResumeWorkflowInput.recoveryTakeover` only when both fields are present on the tracked instance, and only after `ui.confirm` has minted a fresh `AuthorizedByUser` token. `PiWorkflowController.resumeExecution` maps `expectedControllerGeneration` through `createOwnerId` into the engine's `expectedOwnerId` before calling `resumeExecution` (a lease's `ownerId` is set to the controller generation ID at acquire time, so the two are the same underlying value under different names). The Runtime Store and lease semantics remain the sole authority over whether resume actually succeeds; a mismatched, malformed, stale, or terminal correlation still surfaces through the existing `lease_conflict` → `LeaseLost` mapping, and a successful resume always replaces the tracker with the fresh lease the engine returns.
