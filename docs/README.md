...
The public user documentation at [tryweave.io/docs](https://tryweave.io/docs/) is maintained in [pgermishuys/weave-website](https://github.com/pgermishuys/weave-website). The Astro + Starlight package at [`packages/docs/`](../packages/docs/README.md) is an unpublished contributor reference. This `docs/` directory remains the **internal documentation corpus** for architecture, ADRs, specs, and implementation guidance.
...
| [Evals Overview](evals-overview.md) | **Start here for evals.** One page: what evals are for, suites and how each is scored, the Jev judge, the commands to run the dev subset, diagnose one case, repeat and compare runs, trajectory cases and CI dispatch, and how to read results |
...
| [Eval Sanitization and Publish Pipeline](eval-sanitization-and-publish-pipeline.md) | Allowlist sanitizer, deterministic bundle writer, raw artifact writer, token-gated publish policy, `schemaVersion` and freshness rules, the fetch chain of the [tryweave.io/evals](https://tryweave.io/evals) dashboard |
...
| [ADR 0013 — Delegation Targets from Materialized Agents](adr/0013-delegation-targets-from-materialized-agents.md) | **Accepted (2026-09-25)** — Loom and Tapestry are offered only agents the harness holds: the engine drops agents that failed to compose, and adapters report the rest (`materializeAgents({ config, harness })`). See [Spec 38](specs/38-spec-delegation-accuracy/38-spec-delegation-accuracy.md) item 2. |
