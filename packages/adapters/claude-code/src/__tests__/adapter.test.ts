...
describe("ClaudeCodeAdapter", () => {
  ...
  it("spawnSubagent queues agent without writing files", async () => {
    ...
    const result = await adapter.spawnSubagent(makeDescriptor({ name: "loom" }));
  ...
  it("flush writes plugin.json", async () => {
    ...
    const pluginJsonPath = Object.keys(written).find((k) => k.endsWith("plugin.json"));
  ...
  it("flush writes agent markdown files", async () => {
    ...
    const agentPath = Object.keys(written).find((k) => k.includes("agents") && k.endsWith("loom.md"));
  ...
  it("flush writes settings.json when loom agent is present", async () => {
    ...
    const settingsPath = Object.keys(written).find((k) => k.endsWith("settings.json"));
  ...
  it("flush respects custom outDir", async () => {
    ...
    expect(keys.every((k) => k.includes("custom") && k.includes("out"))).toBe(true);
  });
