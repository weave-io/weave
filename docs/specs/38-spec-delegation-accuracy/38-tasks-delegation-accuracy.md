...
| "names only registered shuttles in loom's prompt" (L2) | V1 and V2, every fixture | `shuttle-backend`, `shuttle-frontend` (the prohibition line), `shuttle-core` (the todo-list example) and `shuttle-{category}` (the category paragraph). | Group 3 (3.1) — flipped; group 2's part landed |
| "names only registered agents in tapestry's prompt, outside shuttle-* names" (L2) | V1 and V2, "a builtin and a category shuttle disabled" | `warp`: `<Routing>` says "Do not route plan execution tasks to Pattern, Thread, Spindle, Weft, or Warp" with `warp` disabled. | Group 3 — flipped |
| "names only registered shuttles in tapestry's prompt" (L2) | V1 and V2, every fixture | `shuttle-{category}` in `<Delegation>` and `<Routing>`. | Group 3 (3.4) — flipped; group 2's part landed |

Everything else passes on `main` for both adapters, including the fixtures the audit suspected: a category whose model the harness does not offer is still registered (V1 passes `provider/model` through, V2 registers it without a model since #215) and is correctly offered, and a disabled agent or category shuttle is neither registered nor offered. Not covered by L2: plain-prose mentions. Since group 3 Loom refers to specialists by role, so its prompt no longer says "Warp is mandatory" when `warp` is disabled.
...
## 3. Loom and Tapestry prompt rules — PR: #257
...
- [x] 3.1 `loom.md`: replace the category-shuttle paragraph with "Delegate only to the agents listed above." and the matching-category guidance; remove the example names.
- [x] 3.2 `loom.md`: add the size rule and reconcile the sections it replaces ("Small or self-contained work", the "Never delegate work you can complete correctly in one step" and "Delegate aggressively" lines).
- [x] 3.3 `loom.md` and `tapestry.md`: add the recovery rule (resend once after a transient error; after a configuration error send the task to `shuttle` and tell the user in one line which agent is broken). Fold Tapestry's existing `<ErrorHandling>` retry into it rather than having two rules.
- [x] 3.4 `tapestry.md`: replace the `shuttle-{category}` placeholder in `<Delegation>` and `<Routing>` with "the matching category shuttle listed above".
- [x] 3.5 Copilot: update [`delegation-prompt.ts`](../../../packages/adapters/copilot/src/delegation-prompt.ts) and its tests for the removed placeholder and example line.
- [x] 3.6 Guard tests assert the positive sentences. Flip group 1's (c) known failure; L2 passes on every fixture.
- [x] 3.7 Re-run the cases the size rule touches, at least `loom-delegates-backend-fix-to-category-trajectory` (#230) and `loom-route-pattern-boundary-small-fix`, on the dev subset. Where a case conflicts with the size rule, change the case, not the rule (spec, item notes): widen #230's task to span `src/api/` and `src/db/`, and let a text case accept Loom doing one-place work itself. Record each change and why in the PR.
- [x] 3.8 Update the tryweave.io docs if they describe when Loom delegates. Checked: [tryweave.io/docs/agents](https://tryweave.io/docs/agents/) says only that Loom "decides whether to handle them directly or delegate to specialists" and routes "based on complexity and domain", which the size rule keeps true; no change needed.

**Notes (group 3).**

- **Wording.** Loom and Tapestry say "Delegate only to the agents listed above." Loom's size rule, recovery rule and routing guidance refer to specialists by role ("the codebase explorer", "the strategic planner", "the security auditor"), matching the first words of each builtin's description, rather than by fixed name; `shuttle` is the only fixed name, as the fallback. Tapestry's `<Routing>` no longer lists Pattern, Thread, Spindle, Weft and Warp: "Send a plan task to another listed specialist only when the plan names that agent for the task or the user asks for it." Tapestry's recovery rule replaces its "first failure / second failure" retry.
- **L2.** The item-3 known failures are plain `it`; with group 2 landed, no `it.failing` remains in the contract test.
- **#230.** The task now asks for two fixes, in `src/api/orders.ts` and `src/db/orders.ts`. `loom-route-pattern-boundary-small-fix` passed 3/3 on both dev models with the size rule, so no text case changed.
- **Measurement.** Measured against a fresh baseline on `main` (`bb24e78`), not the 24 Sep one: `--repeat 3`, text track, one unit per suite and dev model, for the three suites whose composed prompts changed (loom-routing, tapestry-execution, tapestry-category-routing). loom-routing: DeepSeek 45/45 → 44/45, GPT 6 Luna 38/45 → 44/45 (+13 points, p = 0.058), no change beyond the noise. tapestry-execution: 11/12 → 12/12 and 12/12 → 11/12, no change. **tapestry-category-routing regressed** on both models (29/30 → 13/30 and 28/30 → 12/30, p < 0.001): its cases describe categories in the user message (`client-frontend`, with legacy `patterns`) that are not in Tapestry's rendered list, since the suite composes Tapestry's prompt from the repository's config. Tapestry now routes those tasks to `shuttle` and says the category's shuttle is not listed, which is the rule working; the suite measured inventing a name from the `shuttle-{category}` placeholder. Making the suite compose each case's categories into Tapestry's list is follow-up work for the maintainer to decide. The three runtime trajectory cases (#230, #231, #232) passed once each on DeepSeek V4 Flash: #230 spawned `shuttle-backend` only, #231 ran two `shuttle` sessions in parallel, #232 fixed the check itself without delegating.
