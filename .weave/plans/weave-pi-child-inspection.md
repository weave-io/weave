...
- [ ] 6. Capture full RPC session state and preserve one-shot settlement
  - **What**: Split `PiRpcChild` into a process/settlement owner plus an injected session observer. Parse and forward every event kind from Task 2. Add typed `steer`, `follow_up`, `get_entries { since }`, and `extension_ui_response` methods guarded by generation, completed authenticated bootstrap, running state, and exact child identity. Spawn persisted children with a private `--session-dir`; restore with the exact `--session` plus active branch/checkpoint context. Preserve force-kill, cancellation races, one pending settlement, and late/duplicate reply handling. Replace the shared 1 MiB native-record/framing failure path with separate incremental handling for native Pi RPC records up to an explicit bounded native-record limit above 1 MiB; do not raise that limit for private control envelopes, which remain capped at 64 KiB. A valid oversized native output must remain private, settle normally, and yield only bounded final output extracted from the terminal assistant response.
  ...
  - **Acceptance**:
    ...
    - Settlement still resolves exactly once and force-kills plus erases transport secrets even while a completed view remains open from history; valid native output larger than 1 MiB cannot cause `ChildSettlementMissing`.
...
- [ ] 7. Build the Pi-native transcript renderer, fallbacks, and extension-UI bridge
...
- [ ] 8. Implement slots, picker, navigation, and per-view editor state
...
- [ ] 9. Integrate every child creation path into one inspection/history registry
...
- [ ] 10. Add ordinary-child recovery and keep workflow resume separate
...
- [ ] 11. Project bounded results, intervention counts, and exports
...
- [ ] 12. Register commands, keys, probes, and package exports
...
- [ ] 13. Prove privacy, quotas, migration, and failure behavior end to end
...
- [ ] 14. Update DSL, architecture, adapter, and package documentation
