...
A direct-step child with eligible targets receives `weave_delegate` but remains outside the ordinary delegation budget itself. Once the direct transport authenticates a running child's `delegate-request`, it MUST relay the request into the generation's shared `PiDelegationController` with the direct child ID, step-agent identity, depth, and cwd supplied by trusted transport state rather than child-controlled fields. The controller MUST resolve the requested target against that step agent's own `delegationTargets`, place the nested helper under the direct child ID, enforce the ordinary depth/concurrency/process/queue limits, and return exactly one correlated `delegate-response`. The helper MUST receive an ordinary bootstrap without `weave_complete_step`. Cancelling the direct child MUST cancel this ordinary descendant subtree as well.
...
Each of `dispatchStep`, `completeStep`, and `reconcileExecution` also returns an ephemeral, top-level `stepPromptText?: string` alongside its effects — the rendered `step.prompt` text for the step about to be dispatched. This is never part of a `LifecycleEffect` (`RunAgentEffect.promptMetadata` continues to carry only `{ byteLength }`), never persisted, and never logged. The controller MUST thread `stepPromptText` from the lifecycle call that produced it straight into the corresponding `runDispatchAgentEffect` call and compose it with the activated agent's own `composedPrompt` (joined by a fixed delimiter) immediately before `directDispatch.dispatch()`; it MUST NOT write the composed or raw text back into an effect, the Runtime Store, a recovery pointer, or a log line.
