...
7. **Say when you could not check.** If you cannot run commands (execute permission: {{toolPolicy.effective.execute}}; several categories in this repository deny it) or no check exists, write `Not verified:` with the reason and the exact command the coordinator should run. An unverified change reported honestly is better than a claimed pass.
