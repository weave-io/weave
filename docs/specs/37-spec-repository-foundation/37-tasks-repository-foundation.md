...
2. 16.3 → 16.4, the judge. Done: 16.3 (#223), 16.4a (#234), and 16.4b (#235), which made Jev the judge.
...
## 16. Truthful scores (G9) — PR: #221 (16.1), #220 (16.2), #223 (16.3), #234 (16.4a), #235 (16.4b), #228 (16.5), #240 (16.6)
...
- [x] 16.3 Jev acceptance check. _Rewritten on 23 Sep 2026: the judge is Jev by design, not the winner of a head-to-head, because a chat-model judge could not join the matrix without grading itself (see the spec's Constraints)._ Make a fresh run on the cheap development subset (17.1) with `--raw-artifacts` on the judge-scored suites: the 11 untagged `task_completion` cases (pattern, weft, warp, spindle, shuttle, tapestry) and the 10 category-routing cases. Select 20 outputs, weighted to weft, warp and pattern. Label each pass or fail. _Revised before any comparison: 18 of the 20 real items were labelled pass, so a judge that passes everything would have met the first rule (16 of 20). 10 constructed negatives were added, each a real item's response with one planted defect, labelled fail._ Score all 30 items with TypeSafe Jev (`typesafe/jev-1.13`, OpenRouter decisions endpoint). Jev is accepted if its agreement with the labels is at least 80% over all 30 items and it correctly fails at least 10 of the 12 fail-labelled items (at most 2 false passes); a judge error counts as a disagreement, never as a false pass. Sonnet 5's verdicts stay in the comparison as a reference only. Record the labels, verdicts, agreement and the result in `docs/artifacts/judge-bakeoff-<date>.md`. The labels double as the human calibration set for later judge changes. _#223: **Jev rejected.** It agreed on 26 of 30 items (87%) but caught only 8 of the 12 fails: it passed three pattern-planning fails (one real, two negatives) and one weft-review negative whose blockers name no file. Sonnet 5, as a reference, gave the same 26/30 and 8/12. Labels came from a blind Claude subagent standing in for the maintainer, plus one maintainer label; see [`judge-bakeoff-2026-09-23.md`](../../artifacts/judge-bakeoff-2026-09-23.md). Jev and Sonnet 5 missed the same four items, so the misses trace to rubrics that do not state those checks; 16.4 makes them explicit and re-runs this identical check once before choosing the judge._
- [x] 16.4 Implement `JevJudge` behind `LangChainJudge` (`langchain-agent-evals.ts`), replacing the hard-coded `JUDGE_MODEL_ID` in `packages/cli/src/commands/eval.ts`. It calls `POST https://openrouter.ai/api/alpha/decisions`, converts each judge-scored case's rubric into Jev questions (`noul` yes/no checks and anchored `score` questions), and derives the rationale from which questions failed, since Jev returns no free text. Pin the judge's version. Record the judge id and version in the provenance manifest and the public report. _18.2 already refuses different judges: it reads `judge: { id, version }` from `bundle-index.json`, else `public-report.json`, else `provenance-manifest.json`, and flags a run without one as "unknown judge". Write it in one of those places._ Only if Jev fails 16.3: use a chat model deliberately kept out of the matrix instead, and record that as a known limitation in `docs/agent-evals.md`. Schema, validate and end-to-end tests change in the same commit as any report schema change. _16.4a (#234): the four items both judges missed in 16.3 traced to checks the rubrics did not state, so the pattern-planning rubrics now require per-task acceptance and no invented commands, and the weft-review rejection rubrics require every BLOCKER line to name a file. The identical check was re-run once on the 10 items whose rubric text changed: **Jev accepted**, 28/30 agreement and 10/12 fails caught (the minimum); Sonnet 5, as a reference, 27/30 and 9/12. The judge is Jev; the fallback is not needed. Jev still passes one blocker-without-file negative and one invented-command negative, so 16.4b's rubric-derived questions should ask those checks directly. See "Re-run with explicit rubric criteria" in [`judge-bakeoff-2026-09-23.md`](../../artifacts/judge-bakeoff-2026-09-23.md)._ _16.4b (#235): `JevJudge` (`jev-judge.ts`) is the judge, pinned to `typesafe/jev-1.13-20260917` in `eval.ts`; an answer from another version is refused. It now reads the agent's actual answer (before, only a summary of which signals fired) with the rubric and reference (`judge-questions.ts`, the acceptance harness's design, pinned by a parity test). It asks one `noul` per criterion and an `overall` noul; the verdict is overall ≥ 0.5, and the rationale names the criteria below 0.5. No `score` question: the acceptance rule never used it. Judge failures are errored cases (`judge-http-failure`, `judge-response-invalid`, `judge-input-too-long`, `judge-input-invalid`), in `tapestry-category-routing` too. `judge: { id, version }` is written to `bundle-index.json`, `public-report.json` (optional strict field), `provenance-manifest.json` and `public-report.md`. The two pattern-planning cases with no command inventory now list their commands. `RealLangChainJudge` stays only for the harness's Sonnet reference. See "The judge" in `docs/agent-evals.md`._
...
- [x] 16.6 The deterministic route reader no longer fails a correct route written as "Primary route: **generic `shuttle`** …; no backend-specific category shuttle is listed." (gpt-6-luna, `loom-route-backend-api`, live in #235): labels accept a `the`/`generic`/`default` qualifier and a negation after an agent stays in its own clause, in `loom-routing` and `tapestry-category-routing`; see "How the route is read out of the answer" in `docs/agent-evals.md`. — PR #240
...
## 6. Understandable and diagnosable evals (G5, G6) — PR: #237 (6.1)

- [x] 6.1 Write `docs/evals-overview.md` (one page): the suites, where cases, rubrics and fixtures live, which runner handles which suite, how scoring works, how a run becomes a published bundle and a website page, and the three commands most people need. Link it from `docs/README.md`, `evals/README.md`, and the top of `docs/agent-evals.md`. _#237: [`docs/evals-overview.md`](../../evals-overview.md), about 150 lines. It covers the suites with their judged and deterministic cases, the Jev judge, the commands (dev subset, single-case diagnosis, `--repeat`, `eval compare`, trajectory, CI dispatch) with the cost and time known so far, reading results, and adding a case or a model. It is linked from all three places._
...
## 7. Fresh baseline (G8) — PR: #241 (7.1), #244 (7.2–7.4), pgermishuys/weave-website#7 and #PRNUM (7.5)
...
- [x] 7.1 Replace the delisted qwen model (`qwen/qwen3.8-max`) in `evals/model-matrix.json` with a listed one (OpenRouter lists `qwen/qwen3.8-max-0902`, checked 23 Sep 2026; since 16.5 every case on a delisted model is reported errored, `model-http-failure`, and the run exits 1), so the full matrix runs without infrastructure errors. _Replaced with `qwen/qwen3.8-max-0902` (checked 24 Sep 2026: every other matrix id is still listed). The tests that named the old id follow. #241._
- [x] 7.2 Run the development subset with repeats on current `main`: `weave eval run --models dev --repeat 3`. It must complete without infrastructure errors. Record its time and cost, and the per-case flip rate (how often a case's verdict differs between repeats; was 19.1). Check OpenRouter credits first. _#244, on `d4d1bf17`, text track (the 5 dev trajectory cases are in 7.3): 276 attempts, 0 errored, 85% passed, $0.18; about 1 h 40 min as one process (it was split into 10 parallel units by suite, 23 min wall clock, because of a 10-minute limit per tool call). 16 of 92 case × model pairs (17%) flipped across the three repeats. See [the baseline](../../artifacts/eval-baseline-2026-09-24.md)._
- [x] 7.3 Run the full default matrix once on current `main`, with the trajectory job (20.2). Record its time and cost. _#244: the text track on all 8 default models (368 attempts, 64 parallel units, 0 errored, 89% passed, 23 min wall clock, $6.92). The trajectory track ran the 5 `opencode-local` cases on every default model they allow: DeepSeek V4 Flash (5/5, $0.04) and Sonnet 4.5, Opus 5 and GPT-5.5 (12/15, 7 min, $3.41), with no harness errors._
- [x] 7.4 Record the pre-WS1 baseline in `docs/artifacts/eval-baseline-<date>.md`: per-suite pass rates, their noise bands, the per-case flip rates, the judge id and version, and the time and cost of each run. Later runs are compared against it with `eval compare` (18.2). Publishing it to the website is not required. _#244: [`eval-baseline-2026-09-24.md`](../../artifacts/eval-baseline-2026-09-24.md), judge `typesafe/jev-1.13-20260917`. It lists three suspected defects without fixing them: `review_blocker_traced` fails on every model, `pattern-plan-verify-by-per-criterion` contradicts the composed Pattern prompt, and a passing trajectory case is labelled `fail`. The bundles stay local; the artifact says where._
- [x] 7.5 Open one weave-website docs PR for the user-visible CLI changes: `--models dev`, `--repeat`, `eval compare` and single-case diagnosis. _pgermishuys/weave-website#7: the CLI reference now covers `--models`, `--repeat`, `--track`, `eval compare`, the Jev judge, errored cases and the current exit codes, and a new Agent Evals guide (`/docs/evals/`) has the dev-subset, diagnose-one-case and measure-a-change recipes. #PRNUM: `weave --help` lists `--models`, and the pipeline doc names the dashboard tryweave.io/evals instead of `/weave-agent-evals/`._
