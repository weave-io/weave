...
- **The case declares them.** `categories` in the case file (`EvalCaseCategorySchema` in `packages/cli/src/evals/types.ts`) lists each category's `name`, `description`, optional `triggers`, and `disabled` (default `false`). The loader rejects duplicate names and unknown keys.
...
- **Provenance still hashes one Tapestry prompt.** The run's prompt snapshot (`prompt-hashes.json`, and the "Prompts" line of `eval compare`) is Tapestry composed from the developer's config, not the per-case prompts, so a change to a case's categories does not show there. `--raw-artifacts` keeps each case's composed prompt.
