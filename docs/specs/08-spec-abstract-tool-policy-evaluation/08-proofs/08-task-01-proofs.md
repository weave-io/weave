...
- The `@weave/core` barrel correctly exports all four tool-policy symbols (`ToolPermission`, `ToolPermissionSchema`, `ToolPolicy`, `ToolPolicySchema`).
...
- Engine code imports from `@weave/core` and does not redefine `allow`/`deny`/`ask` literals.
...
The file imports exclusively from `@weave/core`:
...
This is a value assignment using the `ToolPermission` type from `@weave/core`, not a redefinition of the enum. The `ToolPermissionSchema` enum (`z.enum(["allow", "deny", "ask"])`) remains solely in `packages/core/src/schema.ts`.
...
Typed as `(keyof ToolPolicy)[]` — TypeScript enforces that only valid `ToolPolicy` keys can appear in this array. Adding a sixth capability would require a schema change in `@weave/core` first.
