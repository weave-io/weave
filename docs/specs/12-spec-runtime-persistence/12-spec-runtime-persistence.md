...
- `SessionSnapshot.leaseId` identifies the `ExecutionLease` active when the snapshot was recorded, but is not authoritative for the snapshot's retention: releasing that lease shall never require deleting, blocking on, or failing because of a `SessionSnapshot` that referenced it. The lease-to-snapshot relationship is retention-only metadata, not a lifecycle dependency (see migration v5 below).
...
- `session_snapshots.lease_id` shall be nullable and its foreign key to `execution_leases(id)` shall use `ON DELETE SET NULL` (migration v5, `CURRENT_SCHEMA_VERSION = 5`), not `NO ACTION`/`RESTRICT`. Terminal workflow completion releases (deletes) the driving `ExecutionLease`; that release shall succeed and shall sever the `leaseId` link on any `SessionSnapshot` that observed it rather than deleting the snapshot or failing the release with a foreign key violation. `SessionSnapshot.leaseId` in the domain type is therefore optional (`undefined`/`NULL` for historical snapshots whose lease has since been released), while `RecordSessionSnapshotInput.leaseId` at record time stays required. Since SQLite cannot alter a column's nullability or a foreign key's `ON DELETE` action in place, migration v5 recreates `session_snapshots` (create-copy-drop-rename) with `PRAGMA foreign_keys` disabled for the whole pending-migration transaction only when a pending migration requires it, verified against `PRAGMA foreign_key_check` before commit, and restored immediately after the transaction settles either way. The in-memory Runtime Store mirrors this by severing matching `SessionSnapshot.leaseId` fields when a lease is released, for behavioral parity with SQLite.
