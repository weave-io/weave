...
>
> - **`weft-review-traced-true-positive`.** `review_blocker_traced` needed two file paths on one BLOCKER line. Every model cited the call site's path and named the fallible function (`saveSettings`) by name, which is what the case asks for. A Weft blocker is now also traced when it cites a path and names a function the case's material declares in a different file (`isTracedThroughDeclaredSymbol` in `judgment-cases.ts`); naming only the function that contains the cited line, or a type the case never declares, is still not traced. Warp's `security_finding_traced` is unchanged.
