...
**Allowlist design** — serialization is allowlist-first: only fields declared in `SanitizedCaseResultSummary`, `SanitizedScoreRecord`, etc. appear in publishable output. Unknown fields are silently dropped. This prevents accidental leakage when new fields are added to runner types. `attempt` (a 1-based integer set by the orchestrator under `--repeat`) and `errored` (a boolean), with `errorClassification` (a fixed label from `classifyErrorType()`, only alongside `errored: true`, never provider or scorer text), are allowlisted on `SanitizedCaseResultSummary`; each is written only when set, so a run without repeats or errored cases publishes the same fields as before. See [Repeat cases](./agent-evals.md#repeat-cases---repeat-n) and [Empty and truncated answers](./agent-evals.md#empty-and-truncated-answers-errored-cases).
