...
Pi private-child transcripts and control state are adapter-owned, including child inspection rendering/fallbacks, steering/follow-up, slots/picker/navigation, subtree cancellation, RPC framing and transport, session discovery/path selection, session file I/O, quotas, quarantine, orphan pruning, clear, and recovery orchestration. The Pi adapter may pass only the bounded normalized observations and exports defined by Spec 33 into engine APIs. The engine never scans Pi history and never receives session paths, raw events, checkpoints, or private transcript content. See [Pi](../adapters/pi.md) and [Spec 33](../specs/33-spec-pi-adapter/33-spec-pi-adapter.md).
...
An adapter owns everything concrete: whether its harness exposes a safe per-request seam, which provider and model entries are eligible, how the request is mutated, and what the provider's official response field proves. Only the adapter may report readiness, and it may report `applied` only from correlated response evidence for the same attempt. One adapter-local mapping exists today: the Pi adapter's wrapped OpenAI Codex subscription provider, which reports `requested` or `not-confirmed`. Every other mapping in every adapter reports `unsupported` and sends no control. The distinction is adapter-local in both directions — no engine, config, or core schema knows that a provider, a transport, or a mapping exists. See [Adapter Capabilities](../reference/adapter-capabilities.md#current-provider-fast-support).
