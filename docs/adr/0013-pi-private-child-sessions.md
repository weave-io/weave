...
Issue #21 exposed a related but different recovery concern. added an explicit engine-owned lease takeover for workflow recovery. That does not make private child sessions engine state and must not be reused as a private-session recovery mechanism.
...
4. The exact private-child defaults, limits, commands, controls, privacy exclusions, and failure codes are normative in Pi adapter contract. The adapter recovers only eligible interrupted ordinary top-level children; workflow resume remains an explicit `/weave:resume` engine operation with its existing lease and generation rules.
