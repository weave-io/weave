...
import type { ConfigError, LexError, ParseError, ValidationError } from "../errors.js";
...
describe("LexError variants", () => {
	...
	it("InvalidNumber — holds value field", () => {
		const err: LexError = { type: "InvalidNumber", line: 1, column: 10, value: "1.2.3" };
	...
	it("UnexpectedCharacter — holds char field", () => {
		const err: LexError = { type: "UnexpectedCharacter", line: 2, column: 7, char: "@" };
...
describe("ParseError variants", () => {
	...
	it("MissingBlockName — holds blockType field", () => {
		const err: ParseError = { type: "MissingBlockName", line: 1, column: 1, blockType: "agent" };
...
describe("ConfigError union type guards", () => {
	...
	it("narrows InvalidNumber variant from ConfigError", () => {
		const err: ConfigError = { type: "InvalidNumber", line: 2, column: 4, value: "9.9.9" };
		if (err.type === "InvalidNumber") {
	...
	it("narrows ValidationError variant from ConfigError", () => {
		const err: ConfigError = { type: "ValidationError", path: "x", message: "bad" };
...
describe("formatError", () => {
	it("formats UnterminatedString", () => {
		const result = formatError({ type: "UnterminatedString", line: 3, column: 5 });
		expect(result).toBe("3:5: unterminated string literal");
	});

	it("formats InvalidNumber", () => {
		const result = formatError({ type: "InvalidNumber", line: 1, column: 10, value: "1.2.3" });
		expect(result).toBe("1:10: invalid number literal '1.2.3'");
	});

	it("formats UnexpectedCharacter", () => {
		const result = formatError({ type: "UnexpectedCharacter", line: 2, column: 7, char: "@" });
	...
	it("formats MissingBlockName", () => {
		const result = formatError({ type: "MissingBlockName", line: 1, column: 1, blockType: "agent" });
