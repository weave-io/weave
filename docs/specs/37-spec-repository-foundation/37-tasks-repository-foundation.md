...
- [x] 7.1 Replace the delisted qwen model (`qwen/qwen3.8-max`) in `evals/model-matrix.json` with a listed one (OpenRouter lists `qwen/qwen3.8-max-0902`, checked 23 Sep 2026; since 16.5 every case on a delisted model is reported errored, `model-http-failure`, and the run exits 1), so the full matrix runs without infrastructure errors. _Replaced with `qwen/qwen3.8-max-0902` (checked 24 Sep 2026: every other matrix id is still listed). The tests that named the old id follow._
