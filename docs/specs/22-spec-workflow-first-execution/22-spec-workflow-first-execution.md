...
- `docs/specs/17-spec-workflow-extension/17-spec-workflow-extension.md` already defines generic workflow derivation and anchor-based insertion as a `@weave/config` merge concern. This spec does not replace that mechanism; it adds an engine-visible semantic contract for selected workflows that publish a `before-plan` extension surface after config merge resolution.
