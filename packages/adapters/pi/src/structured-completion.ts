...
export function buildWeaveCompleteStepToolRegistration(deps: {
  ...
  return {
    ...
    // This is a private controller-reporting channel, not a
    // user/agent-governable action (Spec 33 §15): it never requests
    // approval, always resolves to a fixed, closed grant under the
    // `execute` capability the step's own real work already requires.
    resolver: () =>
      ok([
        {
          unresolved: false,
