...
#### `tapestry-category-routing` case composition

A category routes work only because `.weave` declares it: the engine materializes it as `shuttle-{category}` and renders it into Tapestry's `delegation.targets`. So each case in this suite declares its categories, and the runner shows Tapestry exactly those, the way a user who declared them would see them.

- **The case declares them.** `categories` in the case file (`EvalCaseCategorySchema` in `packages/cli/src/evals/types.ts`) lists each category's `name`, `description`, optional `triggers`, and `disabled` (default `false`). The loader rejects duplicate names, unknown keys, and a blank description or trigger (the Weave schema's own rule, so a bad case fails at load, not at composition). Every enabled category's `shuttle-{name}` is also in the case's `allowed_agents`.
- **The runner composes them per case.** `TapestryCasePromptComposer` (`packages/cli/src/evals/tapestry-category-config.ts`) loads the builtin config through `loadConfig()` with a reader that finds no files, so no global or project `.weave` is read; merges a layer holding the case's categories, plus `disable agents ["shuttle-{name}"]` for each disabled one, with `mergeConfigsResult()`; and composes Tapestry with `materializeAgents()`, the path adapters use. Each enabled category is listed as `shuttle-{name}` next to the generic `shuttle`; a disabled one is not listed. Nothing is concatenated into the prompt: what Tapestry reads is what `tapestry.md` renders.
- **The task text no longer restates the config.** Before this, the cases wrote the categories into the task text (in a `patterns [...]` form the DSL no longer has) and never into Tapestry's list. Only `tcr-10-disabled-category` still says, in its task, that the category's shuttle is disabled: a disabled agent is absent from the list, and the case grades whether Tapestry explains the fallback.
- **Provenance records each case's prompt.** The runner hashes every prompt it composed as `tapestry@<caseId>` and returns the hashes on its `RunnerResult` (`promptSnapshots`); the orchestrator records them in `provenance-manifest.json` and `prompt-hashes.json` next to the shared agent snapshots, so `eval compare` names a case whose sent prompt changed, whether through its categories or through `tapestry.md`. The shared `tapestry` record is still Tapestry composed from the developer's config, which is what `tapestry-execution` sends. `--raw-artifacts` keeps each case's prompt text.
- **A composition failure names the case.** If a case's prompt cannot be composed, the run stops before any model call with `PromptProviderFailed` naming the case and the error type (for example `ConfigLoadError`), never the error's raw text.
- **Descriptions reach the model HTML-escaped.** `tapestry.md` renders `{{description}}`, so `/` in a category description arrives as `&#x2F;` (#213). The cases keep their paths in the descriptions, as users write them.

The scenarios in [`tests/evals/category-routing-prompt.scenario.test.ts`](../tests/evals/category-routing-prompt.scenario.test.ts) run the suite with the model and judge stubbed and composition real, and assert that each shipped case's prompt lists exactly its enabled category shuttles.

> **Score change, 25 Sep 2026 (#253).** With [#257](https://github.com/weave-io/weave/pull/257)'s "Delegate only to the agents listed above", this suite fell from about 29/30 to 13/30 on both dev models, because no case's category was in Tapestry's list and Tapestry correctly answered `shuttle`. Composing each case's categories into the list fixes the suite, not Tapestry. Measured on `main` without #257, dev subset, `--repeat 3`: deepseek-v4-flash 29/30 → 30/30, gpt-6-luna 30/30 → 30/30 (no detectable change; this includes the subject-first route reading below). Published scores for this suite may shift slightly between runs on either side of this change; compare only runs on the same side.

#### `tapestry-category-routing` scoring

This suite scores `routingCorrectness` itself, in `tapestry-category-routing-runner.ts`, and takes only the three qualitative dimensions from the scorer (`mergeWithScorerDimensions()`). Routing is graded rather than pass/fail: the expected category shuttle scores 1.0, an accepted alternate 0.8, a fallback to the generic `shuttle` 0.4, and a different category shuttle or no route 0. An answer that only repeats the documentation placeholder `shuttle-{category}` names no shuttle and scores 0.

- **Required cases** pass when `routingCorrectness >= 0.95`. A required case that also declares `transcript_expectations` must then clear the qualitative gate: the average of the qualitative dimensions **that apply to the case** must be at least `QUALITATIVE_PASS_THRESHOLD` (0.7). On an `agent_routing` case — every case in this suite — `delegationCorrectness` and `executionCompleteness` do not apply, so the gate is the judge's `rationaleQuality` verdict against 0.7.
- **Optional cases** pass when `weightedTotal >= 0.5`, where `weightedTotal = routingCorrectness × outcome_weight + (average of the applicable qualitative dimensions) × per_expectation_weight`. When no qualitative dimension applies, routing carries both weights.
...
#### How the route is read out of the answer

`routingCorrectness` in `loom-routing` and `tapestry-category-routing` comes from the route the runner reads out of the answer text, with no judge (`extractRoutedAgents()` in `loom-routing-runner.ts`, `analyzeCategoryRouting()` in `tapestry-category-routing-runner.ts`). These rules keep a correct route from being read as wrong:

- **A label may qualify the agent.** "Primary route: X", "Route: X" and the labelled answers ("Answer: X", "Decision: X", …) accept an optional `the`, `generic` or `default` before X, so "Primary route: **generic `shuttle`**" names `shuttle`. The qualifier does not make anything else a route: X must still be a known agent, and the placeholder `shuttle-{category}` still names none.
- **A negation belongs to its own clause.** A word such as "no" or "is not needed" after an agent negates that agent only within the same clause: in Loom a `;` or `.` ends the window (a comma does not); in Tapestry `. `, `; `, `, ` or a line break does, as its affirmative-route check already did. "Route to `shuttle`; no backend category shuttle is listed" routes to `shuttle`, and "`shuttle` is not needed yet" still rules it out.
- **In Tapestry, the route may be the sentence's subject.** A sentence that opens with a shuttle and "should", "will" or "must" plus handle/take/own/implement/do ("`shuttle` should handle the work; …", "The `shuttle-client-frontend` agent will take this") names the route. "would" and "can" do not, so "`shuttle-backend` would handle API work" is not read as a decision.

> **Score change, 25 Sep 2026 (#253).** Before the subject-first rule, `openai/gpt-6-luna` answered `tcr-10-disabled-category` with "`shuttle` should handle the work; the matching `shuttle-client-frontend` agent is disabled." and scored 0: no affirmative form matched, so the first category shuttle mentioned, the disabled one, was read as the route. Three of six such answers in one diagnostic run were correct routes scored as wrong. Runs scored after the fix can show higher pass rates for this suite on the same answers.

> **Score change, 24 Sep 2026 (Spec 37, task 16.6).** Before this fix both rules were missing, and a correct route could score 0: `openai/gpt-6-luna` answered `loom-route-backend-api` with "Primary route: **generic `shuttle`** to implement the endpoint; no backend-specific category shuttle is listed." and the "no" in the second clause negated `shuttle`; `openai/gpt-6-astra` lost `loom-route-ambiguous-direct-shuttle` the same way ("…stays with Shuttle; no Pattern plan is needed"). Runs scored after the fix can show higher `loom-routing` pass rates on the same answers; compare only runs scored on the same side of it. The format rows are in `tests/evals/suite-runners.scenario.test.ts`.
...
> **Score change, 24 Sep 2026 (Spec 37, task 16.7).** The [pre-WS1 baseline](artifacts/eval-baseline-2026-09-24.md#corrections-24-sep-2026) found two judgment cases no model could pass, for reasons in the case rather than the models:
>
> - **`weft-review-traced-true-positive`.** `review_blocker_traced` needed two file paths on one BLOCKER line. Every model cited the call site's path and named the fallible function (`saveSettings`) by name, which is what the case asks for. A Weft blocker is now also traced when it cites a file path and names a function that, in the case's own code, the cited file calls and the case declares (`extractCodeMaterial` and `isTracedThroughDeclaredSymbol` in `judgment-cases.ts`). Naming only the function that contains the cited line, a type the case never declares, or a function the cited file never calls (a test that only spies on it) is still not traced. Warp's `security_finding_traced` is unchanged.
> - **`pattern-plan-verify-by-per-criterion`.** It required a `verify by:` clause on every acceptance criterion and a `- [ ]` checklist under `## Verification`. #182 removed both from the Pattern prompt on purpose (they fed the verification loops that kept sessions running for hours), and the prompt reserves `- [ ]` for top-level tasks. The case now requires what the prompt asks for: per-task acceptance, a `## Verification` section that names at least one declared command (the new `plan_verification_uses_declared_commands` signal, in any form: list, code block or checklist), and no invented command. Its id is kept so runs stay comparable; `plan_criteria_have_verify_by` and `plan_verification_checkboxes` are still extracted but no case requires them.
>
> Both cases' pass rates rise sharply between runs on either side of this change; compare only runs on the same side.
...
- **`min_parallel_delegations`** (Spec 37, 20.1): the fewest sub-agents named in `expected_spawns` that must run at the same time, from each sub-agent's spawn to its session's completion or error. Tasks dispatched in one step overlap; tasks dispatched one after another do not, because the parent waits for each task to return before its next step. It gates the pass.
...
A case that declares `expected_commands`, a `verifier`, `allowed_delegates` or `min_parallel_delegations` passes only when `executionCompleteness` reaches 0.95, so correct routing alone cannot carry it. Cases without these fields keep the Spec 33 rule (any near-perfect primary dimension passes). Run a local trajectory case with `TMPDIR` pointing at a roomy disk, since each run creates a workspace there:
...
| --- | --- | --- | --- | --- |
| Delegation accuracy | `loom-delegates-backend-fix-to-category-trajectory` (fixture `orders-api`) | Delegations, Configuration delegation failures, Built-in agent delegations, Category-shuttle success | `allowed_delegates: ["shuttle-backend"]`: Loom delegates only to the category shuttle, and the category shuttle edits the code. Since [Spec 38](specs/38-spec-delegation-accuracy/38-spec-delegation-accuracy.md) item 3 the task spans the handler and the data layer (`src/api/` and `src/db/`), because Loom's size rule lets it make a one-place fix itself | Pass on `deepseek/deepseek-v4-flash-0731` (23 Sep 2026); two-file task: pass, spawned `shuttle-backend` only (25 Sep 2026) |
...
- Score buckets (`"pass"` / `"partial"` / `"fail"` / `"skip"`) — human-interpretable, not raw floats. A case's bucket follows its verdict (`computeCaseScoreBucket()`): `"pass"` if it passed, whatever its weighted total; `"partial"` or `"fail"` if it failed, at a total of at least or under 0.5. The explanation says "passed", "failed with a partial score" or "failed" to match. Before 24 Sep 2026 (Spec 37, 16.7) the bucket came from the total alone, so a trajectory case that passed on execution with a total of 0.33 was published as `"fail"`.
