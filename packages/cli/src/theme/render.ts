...
export class ThemeRenderer {
  ...
  renderHelp(theme: ThemeColors): string[] {
    return [
      ...
      `    ${theme.cyan("--agent")} <name>      ${theme.dim("Filter evals to a specific agent")}`,
      `    ${theme.cyan("--model")} <id>        ${theme.dim("Filter evals to a specific model")}`,
      `    ${theme.cyan("--models")} default|dev ${theme.dim("Run evals on the full default matrix or the dev subset")}`,
      `    ${theme.cyan("--case")} <id>         ${theme.dim("Filter evals to a specific case")}`,
      `    ${theme.cyan("--repeat")} <n>        ${theme.dim("Run each eval case n times per model")}`,
      `    ${theme.cyan("--track")} <name>      ${theme.dim("Run only text or trajectory eval cases")}`,
      `    ${theme.cyan("--dry-run")}           ${theme.dim("Preview eval run without executing")}`,
      `    ${theme.cyan("--raw-artifacts")}     ${theme.dim("Emit raw eval artifacts to disk (local-only)")}`,
      "",
