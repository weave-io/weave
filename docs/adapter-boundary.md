...
Type names in `@weaveio/weave-core` and `@weaveio/weave-engine` follow a suffix convention to prevent collisions between the DSL configuration layer and the engine runtime layer:
...
| Concern                                            | Owner                    | Why                                                                     |
| -------------------------------------------------- | ------------------------ | ----------------------------------------------------------------------- |
| `.weave` DSL parsing                               | Core (`@weaveio/weave-core`)     | The DSL is Weave's source of truth                                      |
| Builtin/global/project config merge                | Config (`@weaveio/weave-config`) | Config files are Weave-owned inputs                                     |
| Prompt file path resolution for `.weave/prompts/`  | Config (`@weaveio/weave-config`) | Prompt files are part of Weave config layers                            |
| Prompt composition                                 | Engine (`@weaveio/weave-engine`) | Composition should be reusable and harness-agnostic                     |
| Category shuttle descriptor generation             | Engine (`@weaveio/weave-engine`) | Category shuttles are part of normalized delegation topology            |
| Model intent resolution helper                     | Engine (`@weaveio/weave-engine`) | Pure helper; adapter supplies harness context                           |
...
| PermissionService activation and session authorization | Engine (`@weaveio/weave-engine`) | Service binds project/session/registry/policies and owns repository, clock, ID, and grant internals |
| Permission coverage proof (`verifyPermissionCoverage`) | Engine (`@weaveio/weave-engine`) | Pure helper; adapters supply discovered inventories and interception claims; engine never discovers harness tools |
| Runtime Store permission repository association | Engine (internal) | Memory/SQLite stores register a private repository in an engine-only `WeakMap`; adapters see no permission mutation surface |
...
**Explicit execution operations** (`ExecutionOperationKind`): `start`, `resume`, `pause`, `inspect`, `advance`. These map to `startExecution`, `resumeExecution`, `handleUserInterrupt` (pause signal), `inspectExecution`, and `dispatchStep`/`completeStep` respectively. Only `startExecution` may create a new `WorkflowInstance`. Both `startExecution` and `resumeExecution` may acquire an `ExecutionLease`; by default `resumeExecution` only replaces an already-expired lease. An optional `recoveryTakeover` correlation (`expectedLeaseId`/`expectedOwnerId`) additionally lets `resumeExecution` release and replace one exact, correlated pre-reload lease inside a single atomic Runtime Store transaction — never a broad foreign-lease steal, and never automatic (Issue #21 Task 12 S020; see `docs/specs/33-spec-pi-adapter/33-spec-pi-adapter.md` §18). Because that release affects a lease the caller does not own, the engine requires `authorizationSource` to be the explicit literal `"user"` whenever `recoveryTakeover` is present — a missing or non-`"user"` source fails closed with a typed validation error before any store access.
...
Adapter-facing overview: [Permissions](permissions.md). Normative contract: [Spec 34](specs/34-spec-harness-neutral-permissions/34-spec-harness-neutral-permissions.md).

Spec 34 supersedes one-capability `beforeTool` evaluation as the general contract. `beforeTool` remains a registered workflow compatibility projection over the same permission session; the old one-capability evaluator is exposed as non-authoritative `previewToolPolicy`. There are no legacy `beforeTool` aliases that authorize execution.

Adapters own concrete tool discovery, opaque runtime identities, pure input-aware resolvers, call interception, approval UI/relay, and final permit consumption. The engine owns immutable registry generations, normalized requests, effective-policy binding, approval grants/challenges, short-lived single-use permits, and the harness-neutral coverage proof helper. Adapters activate sessions only through `createPermissionService(runtimeStore).activate(...)`, supplying project identity, controller session ID, the sealed generation, stable effective policies, and schema version. They do not supply a repository, clock, ID source, grant record, identity envelope, or per-call policy. The service retrieves the private repository association from the store and returns an opaque branded session handle.

Before claiming tool-policy readiness, adapters call `verifyPermissionCoverage(context)` with the branded sealed generation plus explicit native, Weave-owned, intercepted, bypassable, and unmanaged third-party inventories. The engine snapshots plain bounded strings once, rejects getters/proxies/extras/duplicates/overlap ambiguity, and returns `PermissionCoverageProof` or a closed `invalid_coverage` / `incomplete_coverage` error. Adapters map either failure onto required `tool-policy-mapping` readiness failure. The engine never discovers harness tools. Concrete registered-tool enforcement wiring remains adapter-owned (issue #21 "Enforce registered-tool policy"); the engine supplies only the neutral proof primitive and fake-adapter tests.

`RuntimeStore` exposes workflow, lease, snapshot, and journal repositories only. Memory and SQLite stores keep durable permission repositories in private fields and register them in an engine-internal `WeakMap`; the association accessor is not a package-root export. This prevents OpenCode and other adapters from mutating durable grants through a store property while preserving durable grants across service sessions.
...
A small number of Weave-owned tools are private controller-reporting channels rather than user/agent-governable actions - the current example is `weave_complete_step` (Spec 33 §15, `structured-completion.ts`), which a direct workflow-step child calls exactly once to report its own completion candidate to its own parent controller. A control-channel tool must remain callable regardless of the descriptor's ordinary `read`/`write`/`execute`/`delegate`/`network` policy - a step descriptor with `execute: deny` must never block the child's own completion report (issue #21 Task 12 exact-host smoke regression: the resolver mapped the call to capability `execute`, the smoke descriptor denied `execute`, `PiPermissionBridge` blocked the call before the recorder ever saw it, and the child fell back to unstructured prose).

This is an adapter-owned exception, not an engine capability change, and it is intentionally narrow:

- **Registration-time opt-in only.** A `PiWeaveToolRegistration` sets `controlChannel: true` (`permission-bridge.ts`) to become *eligible*. This flag alone grants nothing.
- **Live per-call attestation required.** `PiPermissionBridge.intercept()` accepts an adapter-supplied `directStepActive: boolean` for that exact call. The adapter derives it fresh every call from `PiChildModeState.directStep !== undefined` (`extension.ts`) - `undefined` for every ordinary/nested child, set only for the root direct-step child at bootstrap's atomic commit point, and never for anything spawned by that child (nested helpers always bootstrap through the ordinary path, per Spec 33 §15 and `child-control-bodies.ts`'s `DirectStepBootstrapBodySchema`).
- **Live provenance re-verification is unconditional.** `intercept()` always re-validates the exact tool identity's registered source against the live host inventory (`revalidateProvenance`) before considering the control-channel bypass. A displaced, spoofed, or foreign-shadowed registration blocks with `tool-provenance-changed` exactly as it would for any other governed tool - the bypass never weakens this check.
- **Bypass, not blanket allow.** Only when `controlChannel === true` for this exact registered identity *and* `directStepActive === true` for this exact call does `intercept()` skip the engine's `session.authorizeCall` capability-policy gate and return `{ kind: "allow", call }` directly. Every other tool - native or Weave-owned - is unaffected and continues through the ordinary deny/ask/allow evaluation.
- **Fail-closed fallback.** The registration's `resolver` still maps real calls to a normal capability (`execute`, for `weave_complete_step`) so that if the live conditions are ever not met, the tool is governed exactly like any other Weave-owned tool, including blocking under `deny`.
- **Coverage semantics unchanged.** `controlChannelTools` (`PiToolPolicyPlan`) is an adapter-local classification for `intercept()`'s eligibility check only. It is never reported to `verifyPermissionCoverage` as `bypassableToolIdentities` - that engine-level list means "not intercepted at all" (Spec 34 §5/§7) and a control-channel tool is always intercepted; the bypass happens inside `intercept()`, not before it.

See `packages/adapters/pi/src/permission-bridge.ts` (`PiWeaveToolRegistration.controlChannel`, `PiToolPolicyPlan.controlChannelTools`, `intercept()`'s `directStepActive` parameter) and `packages/adapters/pi/src/structured-completion.ts` (`buildWeaveCompleteStepToolRegistration`). Regression tests: `packages/adapters/pi/src/__tests__/permission-bridge.test.ts` (unit-level bypass, no-bypass-without-attestation, no-bypass-without-plan-membership, displaced-provenance-still-blocks), `packages/adapters/pi/src/__tests__/child-mode.test.ts` (end-to-end: execute-deny blocks bash but allows completion, displaced provenance still blocks, an ordinary/nested child never registers or gains completion authority), and `packages/adapters/pi/src/__tests__/structured-completion.test.ts` (`controlChannel: true` on the registration, the tool's own `execute()` records into the recorder).
...
| Concern | Owner | Why |
| --- | --- | --- |
| `PlanStateProvider` interface and `PlanStateError` union | Engine (`@weaveio/weave-engine`) | The engine defines the abstract contract; adapters implement it |
| `validatePlanName` (safe-name regex) | Engine (`@weaveio/weave-engine`) | Path traversal prevention must run before any provider call, regardless of implementation |
| `BunFilesystemPlanStateProvider` (default implementation) | Config (`@weaveio/weave-config`) | Concrete Bun filesystem I/O belongs outside the engine; `@weaveio/weave-config` already owns filesystem I/O for config and prompt files |
...
const result = await completeStep(
  {
    ...
    completionSignal,
    context,
    planStateProvider: new BunFilesystemPlanStateProvider(), // from @weaveio/weave-config
  },
  store,
);
...
The engine owns the canonical denylist of sensitive field names used to sanitize journal entries and session snapshots. This policy is exported from `packages/engine/src/runtime/sanitizer.ts` as `isDeniedKey(key: string): boolean` and re-exported from `@weaveio/weave-engine`.

**Rule**: CLI rendering layers (e.g. `packages/cli/src/commands/runtime.ts`) must import `isDeniedKey` from `@weaveio/weave-engine` rather than maintaining a local copy of the denylist. This ensures the rendering layer and the storage layer apply the same policy.

```ts
// ✅ Correct: import from engine
import { isDeniedKey } from "@weaveio/weave-engine";
