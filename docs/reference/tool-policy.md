...
**Related:** [Adapter Boundary](../architecture/adapter-boundary.md) · [Product Vision](../architecture/product-vision.md) · [Permissions](permissions.md) · [Adapter Capabilities](adapter-capabilities.md) · [ADR 0009](../adr/0009-input-aware-tool-permission-authorization.md)

---

## Purpose

The engine abstract policy composition. When a `.weave` config declares
...
- The raw declared policy is also available so adapters that support policy
  enforcement can apply harness-specific translation.
...
a complete, unambiguous policy. Whether a concrete adapter enforces it is an
...
[tool-policy contract](tool-policy.md)).
...
the bootstrap entry point (see [Adapter Bootstrap Guide](../guides/adapter-development.md)).
...
translation when supported — for example, mapping abstract capabilities to
concrete tool names. The engine never modifies the raw policy before passing it
to the adapter.
...
3. **No harness tool names in engine code.** The engine never hard-codes or
   ...
   `str_replace_editor`). Adapters that support policy enforcement own the
   mapping from abstract capabilities to concrete tool names. See
...
`evaluateEffectiveToolPolicy`, `ConcreteToolClassification`, and `resolveToolDecisions` compose policy intent. They do not authorize an intercepted call. Adapters that claim Weave-enforced `tool-policy-mapping` readiness MUST use [Permissions](permissions.md) via `PermissionService`, registered tools, normalized requests, grants/challenges, single-use permits, and `verifyPermissionCoverage`.

The Pi adapter does not make that claim. It ignores `rawToolPolicy` and
`effectiveToolPolicy`, subscribes to no tool-call interception hook, and leaves
tool authorization to Pi and each concrete tool owner. Its capability report
uses `pi-native-tool-control` to describe that native ownership, not Weave
policy enforcement.

For adapters that implement permission contract, only unrelated third-party tools may remain
`unmanaged` (no Weave permit; never `allow`). Workflow `beforeTool` is the
registered-call compatibility path over the same session; `previewToolPolicy`
is the non-authoritative static preview. See [Permissions](permissions.md) for
ownership, activation, scopes, coverage proof, Runtime Store migration v3, and
test pointers without duplicating permission contract.
...
[Adapter Bootstrap Guide](../guides/adapter-development.md) for the full bootstrap pattern.
...
| [`packages/engine/src/permissions/`](../../packages/engine/src/permissions) | Permission service, registry, session, coverage (see [Permissions](permissions.md)) |
...
| [`packages/engine/src/__tests__/tool-policy.test.ts`](../../packages/engine/src/__tests__/tool-policy.test.ts) | for `evaluateEffectiveToolPolicy` and `resolveToolDecisions`  |
