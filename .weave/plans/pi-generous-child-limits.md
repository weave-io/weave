...
- [x] 10. Preserve strict transport, parser, paging, and allocation fault containment
  ...
  - **Files**: `packages/adapters/pi/src/errors.ts`, `packages/adapters/pi/src/child-envelope.ts`, `packages/adapters/pi/src/child-transfer.ts`, `packages/adapters/pi/src/prompt-chunking.ts`, `packages/adapters/pi/src/child-transcript.ts`, `packages/adapters/pi/src/child-session-checkpoint.ts`, `packages/adapters/pi/src/child-native-sessions.ts`, `packages/adapters/pi/src/__tests__/child-framing.test.ts`, `packages/adapters/pi/src/__tests__/child-envelope.test.ts`, `packages/adapters/pi/src/__tests__/child-transfer.test.ts`, `packages/adapters/pi/src/__tests__/child-session-events.test.ts`, `packages/adapters/pi/src/__tests__/child-native-session-bounded-reads.test.ts`, `packages/adapters/pi/src/__tests__/child-native-session-paging.test.ts`.
...
- [x] 13. Run focused, workspace, migration, performance, and real Pi proof
  ...
  - **Completion record**:
    - Live proof: [`docs/specs/33-spec-pi-adapter/33-proofs/33-task13-generous-child-limits-proof.md`](../../docs/specs/33-spec-pi-adapter/33-proofs/33-task13-generous-child-limits-proof.md), subject `fd1192e`, verdict PASS. It supersedes the earlier completion record below, which was written against the pre-restoration baseline and waived every live case.
    - Exact artifact: `weaveio-weave-adapter-pi-0.0.1-fd1192e-task13.tgz`, SHA-256 `f89eb4962dbfdd4670eeb60c3124675e43eba0de7faf667d32c1a2d41aac6654`. The installed `dist/extension.js`, `dist/index.js`, and `dist/cli.js` digests were re-hashed after installation and match the built artifact. Provenance stayed `npm:@weaveio/weave-adapter-pi`; `WEAVE_PI_UNSAFE_DISABLE_COMMAND_PROVENANCE` was unset.
    - Fresh interactive Pi 0.84.1 reported `Weave adapter mode: ready`, `trust: trusted`, `mode: tui`, `health-only: false`.
    - Real behaviour: ordinary delegation completed; a 46,923-byte (36,123-character) multibyte authoritative task reached the child unchanged, digest `ecb27e19…`; a 178,927-byte authoritative result settled, survived a restart, and reconstructed byte-for-byte through two bounded `children.result` pages (98,304 + 80,623 bytes) to the same digest `8c27b852…`; 21 eligible targets were discoverable and a delegation to the twelfth completed; three children against `max_concurrency 1` started 13.16 s and 13.18 s apart with never more than one live child observed; `/weave:run t13-direct` completed at step `work` and persisted the step's result.
    - Resource measurement (real controller, fake process port): defaults 32/8/8/32 ran 8 concurrent with 24 queued, 0 rejected, peak RSS 96.3 MiB (+14.0 MiB); hard maxima 256/64/32/128 ran 64 concurrent with 128 queued and 64 typed `queue_capacity` rejections, peak RSS 134.7 MiB (+38.4 MiB); no live process after `disposeAll` in either case. One real live child peaked at 204.0 MiB RSS. **No hard-maximum viability is claimed**: 128 concurrent real children were never run.
    - Defects the live proof found and this task fixed: `cba7070` serializes asynchronous control-envelope verification so a multi-chunk large result is no longer rejected as `SequenceMismatch`; `fd1192e` matches durable result identity against the stored path component so direct workflow steps, whose child ids are always hashed, can persist and retrieve their results; `0a6d7fc` removes three event-loop races from the delegation-controller proof and corrects a focused verification command that named a nonexistent test file.
    - Gates at `fd1192e`: `bun test` 8,420 pass / 11 skip / 0 fail; `bun run typecheck`, `bun run lint`, `bun run build`, and `bun run docs:check-links` pass.
    - Cleanup: no residual child process, no active Runtime Store execution lease, and the workflow instance ended `completed`.
    - Superseded record (baseline attempt, retained for history): focused test groups passed; workspace `bun test` reported 8,389 pass, 0 fail, 11 skip; Warp verdict APPROVE; full-suite peak RSS about 2.05 GB. Its Weft remediation removed a durable cumulative 64-run failure in `child-session-refs.ts`, applied the shared 32 KiB UTF-8 diagnostic projection to the protocol `cancel`/`error` reasons, and removed an unrelated `prompt eject` diff. The real Pi cases and the exact-artifact proof were waived at that time and are now superseded by the PASS record above.

## Verification

Status note: every command listed below passed at subject `fd1192e`, including the fresh exact-artifact Pi TUI verification. See the Task 13 completion record for the live proof link and its digests.
...
- `bun test packages/adapters/pi/src/__tests__/child-framing.test.ts packages/adapters/pi/src/__tests__/child-envelope.test.ts packages/adapters/pi/src/__tests__/child-session-events.test.ts packages/adapters/pi/src/__tests__/child-native-session-bounded-reads.test.ts packages/adapters/pi/src/__tests__/child-native-session-paging.test.ts` — source: `[repo: docs/contributing/testing.md]`; proves: strict framing, parser, bounded read, and paging regression coverage.
