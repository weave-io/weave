...
- **`models` ordering in union-merge**: `models` is an ordered preference list consumed by adapters. Union-merge should preserve higher-priority entries first, then append lower-priority entries not already present (project → global → builtin), maintaining the user's declared preference order. **Recommendation**: Adopt this priority-first ordering as the standard for all array union-merges.
- **Architectural note — `@weave/config` package boundary**: This spec creates a separate `@weave/config` package, providing better separation of concerns: config loading is independent of engine lifecycle and adapter translation. This aligns with the product vision that Weave normalizes intent while adapters materialize that intent for a harness.
