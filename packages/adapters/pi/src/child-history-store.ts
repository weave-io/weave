...
const terminal = new Set<PiChildHistoryStatus>([
  "settled",
  "interrupted",
  "quarantined",
  "cleared",
...
function historySettings(
  value: PiChildInspectionEffectiveSettings | HistorySettings,
): HistorySettings {
...
function asStoreError(
  value: PiChildHistoryFsError | PiChildHistoryStoreError,
  action: StoreAction,
...
export type PiChildHistoryStoreError =
  | {
      readonly type: "history-fs";
      readonly action: StoreAction;
      readonly cause: PiChildHistoryFsError;
    }
  | {
      readonly type: "history-json";
      readonly action: "index" | "session";
      readonly reason: "malformed" | "oversized";
    }
  | {
      readonly type: "history-schema";
      readonly reason: "invalid" | "unsupported-version";
    }
  | {
      readonly type: "history-quarantined";
      readonly reason: "index" | "session" | "checkpoint";
    }
  | {
      readonly type: "clear-refused";
      readonly status: "running" | "queued" | "missing";
    }
...
function safeJson(
...
): Result<Uint8Array, PiChildHistoryStoreError> {
  ...
  if (serialized.isErr() || serialized.value === undefined)
    return err(
      serialized.isErr()
        ? serialized.error
        : { type: "history-json", action, reason: "malformed" },
    );
  const bytes = new TextEncoder().encode(serialized.value);
  if (
    bytes.byteLength >
    (action === "index" ? MAX_INDEX_BYTES : MAX_JSONL_LINE_BYTES)
  ) {
...
function decodeJson(
  bytes: Uint8Array,
): Result<unknown, PiChildHistoryStoreError> {
...
function boundedFinalOutput(value: string): string {
  return new TextDecoder().decode(
    new TextEncoder().encode(value).slice(0, MAX_FINAL_OUTPUT_BYTES),
  );
}
...
function canonicalRecord(
  ...
  return {
    ...
    bytes: {
      ...
      total:
        Math.max(0, record.bytes.session) +
        Math.max(0, record.bytes.checkpoint),
    },
  };
}
...
function emptyIndex(
  ...
  return {
    schemaVersion: PI_CHILD_HISTORY_SCHEMA_VERSION,
    parentSessionId,
    records: [],
    updatedAt: now,
  };
...
function migrateV0(
  ...
  parentSessionId: string,
  now: number,
): Result<PiChildHistoryIndexV1, PiChildHistoryStoreError> {
  if (!isRecord(value))
  ...
  for (const raw of source) {
    if (
      ...
      raw.childId.length === 0
    ) {
      return err({ type: "history-schema", reason: "invalid" });
    }
    const status: PiChildHistoryStatus =
      raw.status === "running" ||
      raw.status === "queued" ||
      ...
      raw.status === "cleared"
        ...
        : "settled";
    const kind =
      raw.kind === "nested" || raw.kind === "workflow-step"
        ? raw.kind
        : "ordinary";
    const createdAt =
      ...
      raw.createdAt >= 0
        ...
        : now;
    const finalOutput =
      typeof raw.finalOutput === "string" ? raw.finalOutput : "";
    const migrated: PiChildHistoryRecord = {
      childId: raw.childId,
      parentSessionId,
      parentChildId:
        typeof raw.parentChildId === "string" ? raw.parentChildId : undefined,
      kind,
      status,
      workflow: isRecord(raw.workflow)
        ? {
            ...(typeof raw.workflow.workflow === "string"
              ? { workflow: raw.workflow.workflow }
              : {}),
            ...(typeof raw.workflow.step === "string"
      ...
      activeLeaf:
        typeof raw.activeLeaf === "string" ? raw.activeLeaf : undefined,
      checkpointCursor:
        typeof raw.checkpointCursor === "number" && raw.checkpointCursor >= 0
          ? Math.floor(raw.checkpointCursor)
          : 0,
      branchAncestry: Array.isArray(raw.branchAncestry)
        ? raw.branchAncestry.filter(isRecord).flatMap((branch) =>
            typeof branch.childId === "string" &&
            typeof branch.checkpoint === "number"
              ? [
                  {
                    childId: branch.childId,
                    ...(typeof branch.parentChildId === "string"
                      ? { parentChildId: branch.parentChildId }
                      : {}),
                    checkpoint: Math.max(0, Math.floor(branch.checkpoint)),
                  },
                ]
              : [],
          )
        : [],
      interventionCount:
        typeof raw.interventionCount === "number" && raw.interventionCount >= 0
          ? Math.floor(raw.interventionCount)
          : 0,
    ...
    const checked = parsePiChildHistoryIndex({
      schemaVersion: 1,
      parentSessionId,
      records: [migrated],
      updatedAt: now,
    ...
    if (checked.isErr())
      return err({ type: "history-schema", reason: "invalid" });
    const firstRecord = checked.value.records[0];
    if (!firstRecord) return err({ type: "history-schema", reason: "invalid" });
    records.push(firstRecord);
  }
  return parsePiChildHistoryIndex({
    schemaVersion: 1,
    parentSessionId,
    records,
    updatedAt: now,
  }).mapErr(() => ({ type: "history-schema", reason: "invalid" }));
}
...
export class PiChildHistoryStore {
  ...
  static open(
    ...
    const activeParentSessionId =
      options.activeParentSessionId ?? parentSessionId;
    if (!effectiveSettings.persist_history) {
      const memory = new MemoryPiChildHistoryFs();
      const path = resolvePiChildHistoryRoot(
        parentSessionId,
        options.env,
        options.home,
      );
      ...
      return memory
        .openDirectory(`${path.value}/disabled`, true)
        .andThen((root) =>
          memory
            .openDirectory(`${path.value}/disabled/children`, true)
            .map(
              (children) =>
                new PiChildHistoryStore(
                  parentSessionId,
                  effectiveSettings,
                  memory,
                  root,
                  children,
                  now,
                  emptyIndex(parentSessionId, now()),
                  activeParentSessionId,
                  options.activeChildId,
                  options.inspectedChildId,
                  true,
                ),
            )
            .mapErr((cause) => {
    ...
    const rootPath = resolvePiChildHistoryRoot(
      parentSessionId,
      options.env,
      options.home,
    );
    ...
    const fsResult = fs.openDirectory(rootPathValue, true).andThen((root) =>
      fs
        .openDirectory(
          `${rootPathValue}/${PI_CHILD_HISTORY_LAYOUT.childDirectory}`,
          true,
        )
    ...
    return fsResult
      .andThen(({ root, children }) =>
        readIndex(root, parentSessionId, now())
          .map(
            (index) =>
              new PiChildHistoryStore(
                parentSessionId,
  ...
  getIndex(): PiChildHistoryIndexV1 {
    return this.index;
  }
  getRootPath(): string {
  ...
  private rootIdentity(
    action: StoreAction,
  ): ResultAsync<HistoryIdentity, PiChildHistoryStoreError> {
  ...
  private childDirectory(
    childId: string,
    create = true,
    action: StoreAction = "read",
  ): ResultAsync<PiChildHistoryDirectory, PiChildHistoryStoreError> {
    ...
    return this.children
      ...
      .andThen(() =>
        this.fs
          .openDirectory(`${this.children.path}/${component.value}`, create)
          .mapErr((cause) => mapFs(cause, action)),
      );
  }
  ...
  private persistIndex(
    action: StoreAction = "write",
  ): ResultAsync<void, PiChildHistoryStoreError> {
  ...
  upsertRecord(
    ...
    if (checked.isErr())
      return errAsync({ type: "history-schema", reason: "invalid" });
    const found = this.index.records.findIndex(
      (record) => record.childId === normalized.childId,
    );
    const records = [...this.index.records];
    if (found >= 0) records[found] = normalized;
    else records.push(normalized);
    const parsed = parsePiChildHistoryIndex({
      schemaVersion: 1,
  ...
  updateRecord(
    childId: string,
    patch: Partial<
      Omit<PiChildHistoryRecord, "childId" | "parentSessionId" | "sessionPath">
    >,
  ): ResultAsync<void, PiChildHistoryStoreError> {
    const current = this.index.records.find(
  ...
  appendCheckpoint(
    ...
    return this.withChildDirectory(childId, true, "checkpoint", (directory) =>
      directory
        .readFile(PI_CHILD_HISTORY_LAYOUT.checkpointFile)
        .mapErr((cause) => mapFs(cause, "checkpoint"))
        .andThen((bytes) => {
          const current =
            bytes === undefined
              ? okAsync<PiChildSessionCheckpoint, PiChildHistoryStoreError>(
                  createEmptyPiChildSessionCheckpoint(now),
                )
              : toAsync(decodePiChildSessionCheckpoint(bytes)).orElse(() =>
                  directory
                    .quarantineFile(
                      PI_CHILD_HISTORY_LAYOUT.checkpointFile,
                      "malformed",
                    )
                    ...
                    .andThen(() =>
                      errAsync<
                        ...
                        PiChildHistoryStoreError
                      >({ type: "history-quarantined", reason: "checkpoint" }),
                    ),
                );
          return current
            .andThen((checkpoint) =>
                ...
                appendUnseenCheckpointEntries(
                  ...
                  () =>
                    ({
                      ...
                      action: "session",
                      reason: "oversized",
                    }) as PiChildHistoryStoreError,
                ),
              ),
            )
            .andThen((checkpoint) =>
              toAsync(
                encodePiChildSessionCheckpoint(checkpoint).mapErr(
                  () =>
                    ({
                      type: "history-json",
                      action: "session",
            ...
            .andThen((encoded) => {
              const decoded = decodePiChildSessionCheckpoint(encoded).unwrapOr(
                createEmptyPiChildSessionCheckpoint(now),
              );
              return directory
                .writeFileAtomic(
  ...
  readCheckpointFor(
    ...
    if (this.disabled) return errAsync({ type: "history-disabled" });
    return this.withChildDirectory(childId, false, "read", (directory) =>
      directory
        .readFile(PI_CHILD_HISTORY_LAYOUT.checkpointFile)
  ...
  appendSessionEvent(
    ...
    return this.withChildDirectory(childId, true, "append", (directory) =>
      directory
        .readFile(PI_CHILD_HISTORY_LAYOUT.sessionFile)
        ...
        .andThen(() =>
          directory
            ...
            .mapErr((cause) => mapFs(cause, "append"))
            .andThen(() =>
              this.refreshBytes(childId, directory)
                .andThen(() => this.enforceQuotas())
                .andThen(() => this.persistIndex()),
            ),
        ),
    );
  }
  ...
  clearTerminal(): ResultAsync<number, PiChildHistoryStoreError> {
    if (this.disabled) return errAsync({ type: "history-disabled" });
    const records = this.index.records.filter((record) =>
      terminal.has(record.status),
    );
    return records.reduce<ResultAsync<number, PiChildHistoryStoreError>>(
      (result, record) =>
        result.andThen((count) =>
          this.clear(record.childId).map(() => count + 1),
        ),
      okAsync(0),
    );
  }
  ...
  clear(childId: string): ResultAsync<void, PiChildHistoryStoreError> {
    if (this.disabled) return errAsync({ type: "history-disabled" });
    const index = this.index.records.findIndex(
      (record) => record.childId === childId,
    );
    if (index < 0)
      return errAsync({ type: "clear-refused", status: "missing" });
    const record = this.index.records[index];
    if (!record) return errAsync({ type: "clear-refused", status: "missing" });
    if (record.status === "running" || record.status === "queued")
      return errAsync({ type: "clear-refused", status: record.status });
    return this.withChildDirectory(childId, false, "clear", (directory) =>
      directory
        .deleteFile(PI_CHILD_HISTORY_LAYOUT.sessionFile)
        .andThen(() =>
          directory.deleteFile(PI_CHILD_HISTORY_LAYOUT.checkpointFile),
        )
        .mapErr((cause) => mapFs(cause, "clear")),
    )
      .orElse((error) =>
        error.type === "history-fs" && error.cause.type === "missing"
          ? okAsync(undefined)
          : errAsync(error),
      )
      .andThen(() => {
        this.index = {
          ...this.index,
          records: this.index.records.filter(
  ...
  pruneOrphans(
    now = this.now(),
  ): ResultAsync<number, PiChildHistoryStoreError> {
    if (this.disabled) return okAsync(0);
    const cutoff = now - this.settings.orphan_retention_days * 86_400_000;
    const orphaned = this.index.records.filter(
      (record) =>
        record.parentSessionId !== this.activeParentSessionId &&
        record.updatedAt < cutoff &&
        terminal.has(record.status),
    );
    if (orphaned.length === 0) return okAsync(0);
    return this.deleteMany(orphaned, "prune").andThen(() => {
      const ids = new Set(orphaned.map((record) => record.childId));
      this.index = {
        ...this.index,
        records: this.index.records.filter(
          (record) => !ids.has(record.childId),
        ),
        updatedAt: now,
      };
    ...
    return this.refreshAllBytes()
      .andThen(() => {
        ...
        return this.deleteMany(overChildLimit, "quota").andThen(() => {
          this.markTrimmed(
            new Set(overChildLimit.map((record) => record.childId)),
          );
          return this.refreshAllBytes();
        });
      })
  ...
  private validateSessionBytes(
    ...
    const quarantine = (suffix: "oversized" | "malformed" | "invalid") =>
      directory
        .quarantineFile(PI_CHILD_HISTORY_LAYOUT.sessionFile, suffix)
        .mapErr((cause) => mapFs(cause, "quarantine"))
        .andThen(() =>
          errAsync<readonly PiChildSessionEvent[], PiChildHistoryStoreError>({
            type: "history-quarantined",
            reason: "session",
          }),
        );
    if (bytes.length > this.settings.max_bytes_per_child)
      return quarantine("oversized");
    const decoded = Result.fromThrowable(
      () => new TextDecoder("utf-8", { fatal: true }).decode(bytes),
      () => undefined,
    )();
  ...
  private readPersistedBytes(
    record: PiChildHistoryRecord,
  ): ResultAsync<PersistedBytes, PiChildHistoryStoreError> {
    return this.withChildDirectory(
      record.childId,
      false,
      "quota",
      (directory) =>
          ...
          .andThen((session) =>
            directory
              ...
              .map((checkpoint) => ({
                ...
                total:
                  (session?.byteLength ?? 0) + (checkpoint?.byteLength ?? 0),
              })),
          ),
    ).orElse((error) =>
      error.type === "history-fs" &&
      (error.cause.type === "missing" ||
        (error.cause.type === "unavailable" &&
  ...
  private refreshAllBytes(): ResultAsync<void, PiChildHistoryStoreError> {
    let result: ResultAsync<
      readonly { readonly childId: string; readonly bytes: PersistedBytes }[],
      PiChildHistoryStoreError
    > = okAsync([]);
    for (const record of this.index.records) {
      result = result.andThen((values) =>
        this.readPersistedBytes(record).map((bytes) => [
          ...values,
          { childId: record.childId, bytes },
        ]),
      );
    }
    return result.map((values) => {
      const byChildId = new Map(
        values.map((value) => [value.childId, value.bytes]),
      );
      this.index = {
        ...this.index,
            ...
            const next = {
              ...
              bytes: {
                ...
                checkpoint:
                  ...
                  (checkpoint ? JSON.stringify(checkpoint).length : 0),
                total: (session?.length ?? 0) + (checkpointBytes?.length ?? 0),
              },
              updatedAt: this.now(),
            };
            const records = [...this.index.records];
  ...
  private deleteTerminalBytes(
    record: PiChildHistoryRecord,
    action: StoreAction = "prune",
  ): ResultAsync<void, PiChildHistoryStoreError> {
    return this.withChildDirectory(record.childId, false, action, (directory) =>
      directory
        .deleteFile(PI_CHILD_HISTORY_LAYOUT.sessionFile)
        .andThen(() =>
          directory.deleteFile(PI_CHILD_HISTORY_LAYOUT.checkpointFile),
  ...
  private deleteMany(
    ...
    let result: ResultAsync<void, PiChildHistoryStoreError> =
      okAsync(undefined);
    for (const record of records)
      result = result.andThen(() => this.deleteTerminalBytes(record, action));
    return result;
  }
  ...
  private trimToTotal(
    ...
    const protectBranch = (childId: string): void => {
      ...
      while (current !== undefined && !visited.has(current.childId)) {
        ...
        for (const branch of current.branchAncestry)
          protectedIds.add(branch.childId);
        current =
          current.parentChildId === undefined
            ? undefined
            : byId.get(current.parentChildId);
      }
    };
    ...
    const candidates = records
      .filter(
        (record) =>
          terminal.has(record.status) &&
          !protectedIds.has(record.childId) &&
          record.bytes.total > 0,
      )
      .sort(
        (a, b) =>
    ...
    if (victims.length === 0) {
      return this.persistIndex("quota").andThen(() =>
        total <= this.settings.max_bytes_total
          ? okAsync(undefined)
          : errAsync({
              type: "quota-exceeded",
              scope: "total",
    ...
    return this.deleteMany(victims, "quota")
      ...
      .andThen(() => {
        const finalTotal = this.index.records.reduce(
          (sum, record) => sum + record.bytes.total,
          0,
        );
        return this.persistIndex("quota").andThen(() =>
          finalTotal <= this.settings.max_bytes_total
            ? okAsync(undefined)
            : errAsync({
                type: "quota-exceeded",
                scope: "total",
...
function readIndex(
  ...
  return root
    .readFile(PI_CHILD_HISTORY_LAYOUT.indexFile)
    .andThen((indexBytes) =>
      root.readFile(INDEX_TEMP).map((tempBytes) => ({ indexBytes, tempBytes })),
    )
    .mapErr((cause) => mapFs(cause, "read"))
    .andThen(({ indexBytes, tempBytes }) => {
      if (indexBytes === undefined && tempBytes === undefined)
        return okAsync(emptyIndex(parentSessionId, now));
      const candidates = [
        ...(indexBytes === undefined
          ? []
          : [{ file: PI_CHILD_HISTORY_LAYOUT.indexFile, bytes: indexBytes }]),
        ...(tempBytes === undefined
          ? []
          : [{ file: INDEX_TEMP, bytes: tempBytes }]),
      ];
      let firstFailure: "oversized" | "malformed" | "unsupported" | "invalid" =
        "malformed";
      for (const candidate of candidates) {
        if (candidate.bytes.length > MAX_INDEX_BYTES) {
          firstFailure = "oversized";
          continue;
        }
        ...
        if (isRecord(decoded.value) && decoded.value.schemaVersion === 0) {
          return toAsync(
            migrateV0(decoded.value, parentSessionId, now),
          ).andThen((index) =>
            recoverIndexCandidate(root, candidates, candidate.file, index),
          );
        }
        firstFailure =
          parsed.error.type === "ChildHistoryVersionUnsupported"
            ? "unsupported"
            : "invalid";
      }
      return quarantineIndexFiles(
        root,
...
function recoverIndexCandidate(
  ...
  for (const candidate of candidates) {
    if (candidate.file !== selected)
      result = result.andThen(() => quarantineOneIndex(root, candidate.file));
  }
  return result
    .andThen(() =>
      root
        ...
        .mapErr((cause) => mapFs(cause, "write")),
    )
    .andThen(() =>
      root
        .writeFileAtomic(
    ...
    .andThen(() =>
      root.deleteFile(INDEX_TEMP).mapErr((cause) => mapFs(cause, "write")),
    )
    .map(() => index);
}

function quarantineOneIndex(
  root: PiChildHistoryDirectory,
  file: string,
): ResultAsync<void, PiChildHistoryStoreError> {
  return root
    .quarantineFile(file, "corrupt")
    .mapErr((cause) => mapFs(cause, "quarantine"));
}
...
function quarantineIndexFiles(
  ...
  for (const file of files)
    result = result.andThen(() => quarantineOneIndex(root, file));
