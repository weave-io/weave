...
| 2.6a | Production mapping of §2.6: `child-overlay-pi-native.ts` renders every `PiChildTranscriptEntry` kind in that one style — `❯` prompt, `✻ reasoning · SUMMARY`, `⚙ name(args)` with its `⎿` outcome, `● <child> · reply` with the streaming caret, `↯ queue n`, `· status`, `· usage`, and a full-width `── retry · attempt n ──` run divider. The per-event timestamp column has no authoritative production source and is absent rather than invented. | `renderOverlayPiNative`, `renderOverlayTranscript` |
...
| The SPEND group states the RUN's total — the delegation tree's aggregate for this child, or Pi's own cumulative `UsageTotals` from a standalone `usage` event — and never `message_end.message.usage`. | The pi-ai `Usage` on a terminal assistant message is THAT MESSAGE's accounting. Printing it as the run's spend showed `in 2 · out 101` for a run the parent card scored orders of magnitude higher, and summing those figures would be worse, because every turn re-sends the whole context. The latest per-message report is still retained — it names the model and the context window — it just never becomes a total. An unknown total prints `—`. |
| Money is absent from the live rail unless an authoritative aggregate names it. | `projectAssistantUsageFacts` deliberately projects token counts and no cost figure out of a host usage report, so a live `cost` would have to be derived from tokens. The SPEND group prints `—` instead. |
