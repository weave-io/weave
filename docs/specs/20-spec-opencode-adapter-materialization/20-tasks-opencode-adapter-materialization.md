...
### [ ] 1.0 Establish the injected OpenCode client path and adapter-owned SDK facade
...
- [ ] 1.1 Add explicit adapter constructor options for an injected OpenCode SDK client or narrow adapter-owned facade in `packages/adapters/opencode/src/index.ts`.
- [ ] 1.2 Create `packages/adapters/opencode/src/opencode-client.ts` with a minimal interface for listing agents, creating agents, and updating agents without exposing raw SDK calls throughout the adapter.
- [ ] 1.3 Keep `packages/adapters/opencode/src/sdk-types.ts` as the only direct SDK import surface and add any missing type re-exports needed by the facade.
- [ ] 1.4 Update adapter initialization code so `OpenCodeAdapter` stores the injected dependency without relying on global mutable state.
- [ ] 1.5 Add `packages/adapters/opencode/src/__tests__/adapter.test.ts` coverage proving the adapter can be constructed and initialized with a mocked injected client.
